...
binlog 是单点数据流，多副本部署时每个副本都会全量拉取并重复触发规则链。规则链配置注入[分布式锁](/pages/locker/)后，本端点自动选主：只有主副本读取 binlog，主副本失联后待命副本自动接管，接管后从最新位点（或`fromOldest`配置）继续，间隙事件不回放——与进程崩溃重启的语义一致。单机部署不受影响。