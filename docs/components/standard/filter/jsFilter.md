...
- `jsScript`：用于编写过滤逻辑的JavaScript脚本。该字段内容会被封装到以下函数体中:
  ...
  参数说明:
  - msg：消息内容
    - 当[dataType=JSON](/pages/rule-chain-message/)时，类型为`jsonObject`，可直接使用点号访问属性，如`msg.temperature`
...
2. 脚本执行有超时限制，可通过[config.ScriptMaxExecutionTime](/pages/config/#ScriptMaxExecutionTime)配置
:::
