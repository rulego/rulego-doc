...
| 字段                               | 类型     | 说明                                                                                           | 默认值 |
|----------------------------------|--------|----------------------------------------------------------------------------------------------|-----|
| functionName                     | string | 调用的函数名称。可以使用[组件配置变量](/pages/component-configuration-variables/) 动态指定函数名                                                 | 无   |
| param   <Badge text="v0.35.0+"/> | string | 函数入参。支持表达式取值进行替换，示例：`${msg.value}`、`{"name":"${msg.name}","type":"admin"}`。如果空，则使用当前消息负荷作为参数 | 无   |
...
- [config.Udf](/pages/config/#udf) ：js脚本运行时调用的自定义函数，函数没固定参数和返回值。
- [functions](/pages/functions/) ：`functions`节点通过配置函数名调用的自定义函数，函数必须是该类型：`func(ctx types.RuleContext, msg types.RuleMsg)`。