...
---
`cacheSet`组件：设置指定key的值到缓存中。缓存实例使用[config.Cache](/pages/config/#cache) 。

## 配置

| 字段    | 类型          | 说明                                 | 默认值   |
|-------|-------------|------------------------------------|-------|
| items | []CacheItem | 缓存项列表，可以使用[组件配置变量](/pages/component-configuration-variables/) | 无     |
...
| value | any    | 值，可以使用[组件配置变量](/pages/component-configuration-variables/)                                    | 无     |
