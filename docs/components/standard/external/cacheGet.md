...
---
`cacheGet`组件：从缓存中获取指定key列表的值。缓存实例使用[config.Cache](/pages/config/#cache) 。
...
| whenKeyNotFound | string     | 找不到key时的处理策略（忽略大小写）        | 空   |

- **outputMode**: 输出模式
    - 0:查询结果，合并到当前消息元数据
    - 1:合并到当前消息负荷。要求输入消息负荷`DataType`必须是JSON类型，并且消息负荷`Data`可以解析为map结构
    - 2:查询结果，转成JSON，覆盖原消息负荷输出
- **whenKeyNotFound**: 找不到key时的处理策略，不区分大小写
    - 空或未设置：保持原有行为（Mode 0/1 走成功链，Mode 2 全部 miss 走失败链）
    - failure：找不到key时走失败链
    - success：找不到key时走成功链
...
| key   | string | 键，可以使用[组件配置变量](/pages/component-configuration-variables/)                                    | 无     |
...
- ***Failure:*** 获取失败（包括底层缓存客户端报错）或根据`whenKeyNotFound`配置未找到缓存时，把消息发送到`Failure`链
