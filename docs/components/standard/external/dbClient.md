...
## 配置
该组件允许通关过`dsn`字段复用共享的连接客户端。参考[组件连接复用](/pages/component-connection-reuse/) 。

| 字段                                | 类型     | 说明                                         | 默认值   |
|-----------------------------------|--------|--------------------------------------------|-------|
| opType   <Badge text="v0.34.0+"/> | string | SQL操作类型，INSERT/UPDATE/DELETE/SELECT，自动自动检测 | ''    |
| sql                               | string | SQL语句，可以使用[组件配置变量](/pages/component-configuration-variables/)         | 无     |
| params                            | 数组     | SQL语句参数列表，可以使用[组件配置变量](/pages/component-configuration-variables/)     | 无     |
