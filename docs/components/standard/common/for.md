...
| 字段    | 类型     | 说明                                     | 默认值  |
|-------|--------|----------------------------------------|------|
| range | string | 遍历的目标表达式,示例：${msg.items}，支持[组件配置变量](/pages/component-configuration-variables/)                              | 1..3 |
...
:::tip
**上下文隔离与传递：**
- 当 `mode` 为 `0` 或 `1` 时，每次迭代的起始输入都是**初始的原始消息**（仅 `Data` 被替换为当前遍历项），迭代之间**上下文相互隔离**。
- 当 `mode` 为 `2`（覆盖）时，上一次迭代修改后的结果（包括 Metadata）会**传递给下一次迭代**。
:::
