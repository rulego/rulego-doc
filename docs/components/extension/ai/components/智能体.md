...
| tools | []Tool | 工具列表配置。详见 [工具系统](/pages/ai-agent-tools/) | |
...
- **GLM / Claude 原生风格**（部分兼容层）：可能先输出一段说明文本再发起工具调用。此类前导文本通常很短（如「我查一下…」），自动模式的观察窗口即可覆盖；仅当前导文本超过窗口仍接工具调用时才需要 `drain`

排障：如果流式回答正常但**工具从未执行**，且日志出现 `model emitted tool calls after content`，说明模型的前导文本超出了观察窗口——把 `streamToolCallCheck` 设为 `drain`。
