...
---
`x/iotRead`组件：<Badge text="v0.37.0+"/> **统一采集入口**。按 `driver` 切换协议后端（modbus / s7 / eip / snmp / opcua / mc / fins / dlt645 / iec104），配置项透传给对应协议的读节点。输出统一为 `iot_points.Data`，与具体协议解耦--换后端只改 `driver` 与该协议的连接/点位配置，链上下游不变。
...
- 落盘统一入口：[x/tsdbWrite](/pages/x-tsdb-write/)（配置 measurement 后可直接接收采集点数组）
