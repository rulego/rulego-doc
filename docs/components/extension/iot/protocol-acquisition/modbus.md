...
| regType            | uint   | 寄存器类型，可选值：<br>`0` - 保持寄存器（HOLDING_REGISTER）功能码：0x03<br>`1` - 输入寄存器（INPUT_REGISTER）功能码：0x04             | 无                     |
| **TcpConfig**      |        | **TCP 配置参数**                                                                                           |                       |
| timeout            | int64  | 请求超时时间，单位为秒                                                                                            | `5`                   |
| certFile           | string | 证书路径（TLS/SSL连接时需要提供）                                                                                   | 无                     |
| certKeyFile            | string | 私钥路径（TLS/SSL连接时需要提供）                                                                                   | 无                     |
| caFile             | string | CA证书路径（TLS/SSL连接时需要提供）                                                                                 | 无                     |
| **RtuConfig**      |        | **RTU 配置参数**                                                                                           |                       |
| speed              | uint   | 串行波特率（仅RTU模式）                                                                                          | `19200`               |
| dataBits           | uint   | 数据位数（仅RTU模式）                                                                                           | `8`                   |
| parity             | string | 校验位（仅RTU模式），可选值：<br>`N` - 无校验<br>`E` - 偶校验<br>`O` - 奇校验<br><br>全称 `none`/`even`/`odd` 亦可，大小写不敏感。<br>为兼容早期配置，数值 `0`/`1`/`2` 仍可读入（依次对应 N/E/O），但导出与文档统一用字母。<br><br>注：字母形式对齐现场惯用写法（如 `8E1`）。RTU 驱动不支持 Mark/Space 校验，填 `M`/`S` 会明确报错而非静默回落——需要这两种请用 [x/serial](/pages/x-serial/) 组件。 | `N`                   |
