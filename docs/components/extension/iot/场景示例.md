...
`x/control/timer`（定时器）与 `x/control/watchdog`（看门狗）是协议无关的逻辑组件，在规则链里与 9 协议的读写节点串接，即可实现软PLC式的延时动作与失联保护。组件文档：[控制定时器](/pages/x-control-timer/)、[看门狗](/pages/x-control-watchdog/)。
