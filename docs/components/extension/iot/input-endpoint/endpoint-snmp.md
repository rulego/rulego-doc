...
---
***SNMP 告警端点*** <Badge text="v0.37.0+"/> 监听 UDP 162 端口，被动接收网络/设施设备（交换机/路由器/UPS/传感器等）主动推送的 SNMP Trap/Inform 告警，转成消息流入规则链处理。无需轮询，适合设备告警场景（如链路故障、UPS 低电、温度超限等），与主动采集组件 [`x/snmpRead`](/pages/x-snmp-read/) 互补。
