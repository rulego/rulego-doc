...
| 组件 | 说明 |
|------|------|
| [x/control/timer](/pages/x-control-timer/) | 软PLC 定时器（TON 接通延时 / TOF 断开延时），可取消、可重触发 |
| [x/control/watchdog](/pages/x-control-watchdog/) | 看门狗，消息透传并重新武装，失联超时下发故障安全 JSON |
...
聚合 SQL 用 `GROUP BY name` 逐点统计；需跨点位计算时给聚合节点配置 `inputFormat: columns`（透视为宽表）。完整示例见 [IoT 场景示例](/pages/iot-scenarios/)。
...
更多场景组合示例见：[IoT 场景示例](/pages/iot-scenarios/)
