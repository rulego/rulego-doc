...
该组件支持通过`server`字段复用共享的NSQ连接客户端，避免重复创建连接。详见[组件连接复用](/pages/component-connection-reuse/)。
...
| topic     | string | 是   | 发布主题，支持使用[组件配置变量](/pages/component-configuration-variables/)进行动态配置                  | device_msg     |
