...
| collection | string | 是   | 集合名称，支持使用[组件配置变量](/pages/component-configuration-variables/)进行动态配置                   | 无     |
| opType     | string | 是   | 操作类型，可选值：INSERT(插入)、UPDATE(更新)、DELETE(删除)、QUERY(查询)     | QUERY |
| filter     | string | 否   | 过滤条件，JSON格式，如{"age": {"$gte": 18}}。支持使用[组件配置变量](/pages/component-configuration-variables/)进行动态配置      | 无     |
| doc        | string | 否   | 更新/插入的文档内容，JSON格式，如{"name":"test","age":18}。支持使用[组件配置变量](/pages/component-configuration-variables/)进行动态配置  | 无     |
