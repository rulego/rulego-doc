...
| topic     | string | 是   | 发布主题，支持使用[组件配置变量](/pages/component-configuration-variables/)进行动态配置                  | /device/msg               |
| key | string | 否   | 消息键模板，支持使用[组件配置变量](/pages/component-configuration-variables/)进行动态配置，用于消息路由和分区    | 无                        |
| headers   | map[string]string | 否   | 自定义消息属性，支持key和value都使用[组件配置变量](/pages/component-configuration-variables/)进行动态配置 | 无                        |
