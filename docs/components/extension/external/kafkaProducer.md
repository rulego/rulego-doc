...
该组件支持通过`server`字段复用共享的Kafka连接客户端，避免重复创建连接；共享连接可同时供`endpoint/kafka`端点与发布节点复用（在编辑器「共享连接」下拉中互相可选）。详见[组件连接复用](/pages/component-connection-reuse/)。
...
| key       | string | 否   | 消息分区键，支持使用[组件配置变量](/pages/component-configuration-variables/)进行动态配置。用于控制消息分区分配    | 无                  |
