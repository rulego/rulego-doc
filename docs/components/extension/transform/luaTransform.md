...
- `script`：可以对msg、metadata、msgType进行转换。只需要提供函数体内容，如果是文件路径，则需要提供完整的脚本函数：
  ...
  ```
  - msg：消息内容，如果[dataType=JSON](/pages/rule-chain-message/)，可以使用`msg.temperature` 方式操作。如果dataType是其他类型，该字段类型是:`string`
...
可以通过`config.RegisterUdf`注册golang函数，然后在Lua脚本中使用，其注册方式和用法和Js一致。详情参考[Udf](/pages/config/#udf) 。
