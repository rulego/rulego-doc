...
数据源不是 MQTT、而是 PLC/电表等工业设备时，用 [rulego-components-iot](/pages/iot-overview/) 提供协议采集（`x/iotRead`，支持 Modbus/S7/OPC UA/FINS 等 9 种协议）。采集输出的点位数组可**直连** `x/streamAggregator`（`GROUP BY name` 逐点统计；配置 `inputFormat: columns` 则透视为宽表做跨点位计算），聚合结果由 `x/tsdbWrite` 落盘：

```
x/iotRead → x/streamAggregator → x/tsdbWrite
```

完整规则链 DSL 见 [IoT 场景示例 · 采集 → 聚合 → 落盘](/pages/iot-scenarios/)。