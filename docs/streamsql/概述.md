...
- 🔍 **分析函数与变化检测**：`lag` / `had_changed` / `changed_col(s)` / `latest` / `acc_*` 等跨事件状态函数，支持 `OVER (PARTITION BY/WHEN)`，专攻 CDC 变化检测、上下文回溯、生命周期累积。详见[分析函数](/pages/streamsql-analytic/)。
- 🧩 **模式识别（CEP）**：`MATCH_RECOGNIZE` 识别事件序列模式——连续越限防抖、升温骤降、开停机工作流、乱序事件、时间窗约束。SQL:2016 标准语法（对齐 Flink），`PATTERN`/`DEFINE`/`MEASURES`/导航/聚合/`SUBSET`/`FINAL`/`WITHIN` 全支持，四道闸保证边缘内存有界。详见[模式识别](/pages/streamsql-cep/)。
...
**何时用什么**：相邻事件比较（变没变）→ 分析函数；事件顺序/序列模式（A 之后接 B、连续 N 次）→ CEP；时间段统计（每分钟均值）→ 窗口聚合 + HAVING。详见[模式识别](/pages/streamsql-cep/)与[分析函数](/pages/streamsql-analytic/)。
