...
- **权衡**：IdleTimeout>0 时空闲期水位按 `now - maxOutOfOrderness` 前推，能及时关闭空闲窗口、清理状态；但空闲后到达的、事件时间早于前推水位的**老事件会被当迟到丢弃**。不能容忍老事件丢失则保持 IdleTimeout=0（此时空闲的事件时间窗口会挂起直到新数据到达）
...
此时可通过 `WITH(STATETTL='24h')` 为计数窗口开启状态 TTL，定期清理长期不活跃的"死 key"。默认 `0`（禁用，与 Flink 行为对齐）。详见 [SQL参考 - STATETTL](/pages/streamsql-sql/#statettl)。
:::
...
- 基于数据间隔判断会话
- 适合用户行为分析
- 当前 session 窗口**仅做尾部扩展**（gap 内新事件延长会话 End），**不做跨 session 合并**（Flink 的 merging）。仅事件时间 + 严重乱序数据下与 Flink 行为不同
...
::: warning 内存风险与 State TTL
全局窗口比时间/计数窗口风险更高——它没有任何边界。系统只维护每个分组的运行态聚合（`O(分组数 × 聚合态)`，不缓存原始行），但若 `GROUP BY` 基数很高、而 `TRIGGER WHEN` 又极少命中，未触发的分组会持续累积导致内存增长。

此时用 `WITH(STATETTL='1h')` 开启状态 TTL，后台定期清理长期不触发的“死分组”。默认 `0`（禁用，与 Flink 行为对齐）。详见 [SQL参考 - STATETTL](/pages/streamsql-sql/#statettl)。
:::
...
- 对聚合查询调用 `EmitSync` 会报错（同步模式只支持非聚合查询）；非聚合查询也可改用 `Emit` + `AddSink` 走异步。
- **分析函数**（`lag` / `had_changed` / `changed_col` / `changed_cols` / `latest` / `acc_*`）属于**非聚合**查询——逐事件维护跨事件状态、每条数据到达即求值，可用在 `SELECT` 和 `WHERE`，典型用途是变化检测（CDC）。详见 [分析函数](/pages/streamsql-analytic/)。
- 分析函数**可用于窗口查询**（对窗口产出行求值，参数可为聚合函数），但**不能进 `HAVING`**；时间段/持续检测用窗口聚合 + `HAVING`（见[滑动窗口与持续检测](/pages/streamsql-case-sliding/)）。
