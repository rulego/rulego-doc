...
::: tip 如何运行这些 SQL
本文每个场景只给 SQL + 输入 + 输出；运行方式见[案例集锦 · 如何运行案例 SQL](/pages/streamsql-cases-overview/#如何运行案例-sql)。**模式识别查询必须用 `Emit` + `AddSink`**（`EmitSync` 不支持 MATCH_RECOGNIZE）；想保序用 `AddSyncSink`。
:::
...
- [模式识别（MATCH_RECOGNIZE）](/pages/streamsql-cep/) —— 完整语法参考（PATTERN / DEFINE / MEASURES / 导航 / 聚合 / SUBSET / FINAL-RUNNING / SKIP）
- [如何运行案例 SQL](/pages/streamsql-cases-overview/#如何运行案例-sql) —— 统一的 Go 运行样板
- [分析函数](/pages/streamsql-analytic/) —— 相邻两事件的变化检测（`lag`/`had_changed`），与 CEP 互补
