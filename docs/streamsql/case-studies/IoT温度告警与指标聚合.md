...
::: tip 如何运行这些 SQL
本文每个场景只给 SQL + 输入 + 输出；运行方式见[案例集锦 · 如何运行案例 SQL](/pages/streamsql-cases-overview/#如何运行案例-sql)。**非聚合**查询（场景 A）用 `EmitSync` 同步拿单条结果；**聚合/窗口**查询（场景 B–E）用 `Emit` + `AddSink` 按批回调。
:::
...
- [如何运行案例 SQL](/pages/streamsql-cases-overview/#如何运行案例-sql) —— 统一的 Go 运行样板
- [变更数据捕获](/pages/streamsql-cdc/) —— 分析函数做变化检测
