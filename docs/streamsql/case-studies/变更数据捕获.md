...
在 IoT 场景里，"变更数据捕获"指**从连续的设备上报流里识别出真正发生变化的事件**——指标突变跨越阈值、状态翻转、字段变化等。StreamSQL 的[分析函数](/pages/streamsql-analytic/)（`lag` / `had_changed` / `changed_col` / `changed_cols` / `latest` / `acc_*`）专为这类逐事件的状态计算设计：每条数据到达立刻求值，状态跨事件保留，可用在 `SELECT` 和 `WHERE`。
...
- [IoT温度告警与指标聚合](/pages/streamsql-case-iot-temperature/) - 窗口聚合类 IoT 案例
