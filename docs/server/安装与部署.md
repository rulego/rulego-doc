...
从 [Github Releases](https://github.com/rulego/rulego/releases/) 下载对应平台的二进制包，两种包的区别：

- **`rulego-server-all-<平台>.zip`**：内置可视化编辑器（推荐，开箱即用）
- `rulego-server-<平台>.zip`：纯后端，不含编辑器（体积小，适合纯 API/嵌入场景，编辑器需按下文方式单独安装）

解压后得到 `server` 二进制和 `config.conf` 配置文件。
...
::: tip
下载的 `rulego-server-all-*` 包已内置编辑器，无需本节操作。仅使用普通包或从源码编译时才需要单独安装。
:::

### 方式一：使用预编译包

1. 从 [Github Releases](https://github.com/rulego/rulego/releases/) 下载 `rulego-server-all-<平台>.zip`，解压取出其中的 `editor` 文件夹
2. 将 `editor` 文件夹放到 server 二进制文件**相同目录**下：
...
| `msg_max_hops` | `10000` | 单条消息在引擎内允许经过的最大节点跳数（含子链）。超限消息以 `Failure` 终止并记录 Error 日志，兜底 `while` 条件恒真、链成环、子链递归等消息不终止场景。0 或负数表示不限制 |
| `worker_pool_max_workers` | `5000` | 每用户引擎工作协程池上限。池满后新任务在调用方协程同步执行形成背压，goroutine 总量有上界。0 或负数表示不限制 |
...
**GOMEMLIMIT（推荐）**：Go 1.19+ 支持 `GOMEMLIMIT` 环境变量，可硬性限制堆内存上限，防止在无 swap 的受限设备上 OOM：

```bash
# 设置堆上限 256MiB（默认 GOGC=100，不影响吞吐）
GOMEMLIMIT=256MiB ./server -c=./config.conf
```

实测数据（16 核 / 32GB，规则链 js→filter→switch→restApiCall）：

| 配置 | 吞吐 | RSS 峰值 | 适用场景 |
|------|------|----------|----------|
| 默认（GOGC=100） | 65k/s | 134MB | 通用 |
| GOGC=200 | 76k/s（+16%） | 179MB | 内存充裕，追求吞吐 |
| GOMEMLIMIT=256MiB | 88k/s（+35%） | 303MB | 受限设备，内存可控 |
| GOMEMLIMIT=512MiB | 92k/s（+40%） | 539MB | 内存充裕，极致吞吐 |

> ⚠ 少核机器（≤2 核）**勿用 `GOGC=off`**：贴顶 GC 会吃掉稀缺 CPU，延迟毛刺可达数秒。建议保持默认 GOGC + GOMEMLIMIT 做安全上限。
...
| 标签 | 说明 |
|------|------|
| `with_all` | 所有可选组件（等同于同时启用下面全部标签，**不含** `use_fasthttp`） |
| `with_ai` | AI 相关组件（ai/agent 节点、LLM 工具等） |
| `with_iot` | IoT 相关组件 |
| `with_etl` | ETL 数据处理组件 |
| `with_ci` | CI/CD 相关组件 |
| `with_extend` | 扩展组件 |
| `use_fasthttp` | 用 fasthttp 替换 HTTP 端点和 restApiCall 组件（需与 `with_all` 等组合使用） |

> **`use_fasthttp` 选型建议**：按负载特征选型，与机器核数关系不大（2 核小机实测同样正向收益）：
>
> - **短连接多、高并发、延迟敏感**的场景收益最大：吞吐提升约 20%~30%，高并发下 p99 延迟降低约 35%~45%（同机同链 A/B 实测）
> - 常规 keep-alive API：吞吐提升约 7%~10%
> - 业务链本身耗时较重（如复杂脚本节点）时，端点层收益会被业务耗时摊薄
>
> 注意：需使用包含 fasthttp 端点修复的 rulego-components（否则链 DSL 中 `"type": "http"` 会因注册别名丢失而加载失败）；生产部署建议 `CGO_ENABLED=0` 静态编译。用法示例：
>
> ```bash
> # 启用 fasthttp 端点
...
# 单条消息最大节点跳数（含子链），超限以 Failure 终止，0 表示不限制
msg_max_hops = 10000
# 每用户引擎工作协程池上限，池满后新任务同步执行形成背压，0 表示不限制
