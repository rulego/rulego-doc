...
| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| GET | `/api/v1/overview` | `rule:read` | 获取当前用户的总览统计（规则链数量、状态分布等） |
| GET | `/api/v1/version` | 认证 | 获取服务端版本信息 |

## 用户管理

| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| GET | `/api/v1/users/me` | 认证 | 获取当前登录用户信息 |
| PATCH | `/api/v1/users/me` | 认证 | 修改当前用户信息 |
| GET | `/api/v1/users` | `user:read` | 列出所有用户（admin） |
| POST | `/api/v1/users` | `user:write` | 创建/更新用户（admin） |
| DELETE | `/api/v1/users/:targetUsername` | `user:delete` | 删除用户（admin，`?purge=true` 清除数据） |

详见 [用户认证与权限](/pages/rulego-server-auth/)。
...
| GET | `/api/v1/category/rule` | `rule:read` | 获取规则链分类树 |
| GET | `/api/v1/rules/:id` | `rule:read` | 获取规则链 DSL |
| GET | `/api/v1/rules/:id/latest` | `rule:read` | 获取当前用户最近修改的规则链（`:id` 可传 `_`） |
| GET | `/api/v1/rules/:id/status` | `rule:read` | 获取规则链运行状态 |
...
响应 `items` 为摘要项（`ruleChain` 的 id/name/root/disabled/additionalInfo 摘要 + 首个 endpoint 类型），
不含完整 DSL，完整定义通过 `GET /api/v1/rules/{id}` 获取。手动上传到服务器数据目录的 DSL 文件
会在下次列表请求时自动同步进索引。
...
|------|------|
| `chainId` | 规则链 ID |
| `flowType` | 流向：`In`（节点接收数据）/ `Out`（节点处理完成） |
| `nodeId` | 节点 ID |
| `relationType` | 关系类型（Success、True、False 等） |
| `err` | 错误信息 |
| `msg` | 消息内容 |
| `ts` | 时间戳（毫秒） |
...
| GET | `/api/v1/shared-node-statuses` | `component:read` | 获取共享节点运行状态 |
...
| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| POST | `/api/v1/builtins/:name` | `config:read` | 执行内置处理器（如脚本预检、表达式求值等） |
...
| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| GET | `/api/v1/iot/point-templates` | `config:read` | 列出点位模板 |
| GET | `/api/v1/iot/point-templates/:id` | `config:read` | 获取点位模板 |
| POST | `/api/v1/iot/point-templates` | `config:write` | 创建点位模板 |
| PUT | `/api/v1/iot/point-templates/:id` | `config:write` | 更新点位模板 |
| DELETE | `/api/v1/iot/point-templates/:id` | `config:write` | 删除点位模板 |
