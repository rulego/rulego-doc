...
::: warning
Struct tag 必须写在同一行内。Go 的反射解析遇换行即停止，tag 拆成两行会静默丢失，可视化表单将不渲染该字段。
:::

**下拉选择：**

```go
RequestMethod string `json:"requestMethod" label:"请求方法" component:"{\"type\":\"select\",\"options\":[{\"label\":\"GET\",\"value\":\"GET\"},{\"label\":\"POST\",\"value\":\"POST\"}]}"`
```

**代码编辑器：**

```go
JsScript string `json:"jsScript" label:"脚本" component:"{\"type\":\"codeEditor\",\"language\":\"javascript\"}"`
```
```
...
- [自定义组件概述](/pages/custom-components-overview/) — 组件接口和开发指南
