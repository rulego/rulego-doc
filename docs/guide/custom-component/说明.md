...
```go
// Node 节点组件接口
//把业务逻辑或者通用逻辑封装成组件，然后通过规则链配置方式调用该组件
type Node interface {
  //New 创建一个组件新实例
...
连接型组件（MQTT 客户端、数据库、串口、工业协议客户端等）可以暴露自身的连接状态，供规则链状态聚合、可视化监控、运维告警使用。状态是**只读查询**，不会触发建连。

`StatusInfo` 描述一个连接的当前状态：

```go
type StatusInfo struct {
    Status  NodeStatus // connected / reconnecting / disconnected / none
    Message string     // 最后一次错误信息，通常在 reconnecting 时填写
}
```
...
|----|------|
| `StatusConnected` | 已连接 |
| `StatusReconnecting` | 连接丢失，重连进行中（含首次连接失败的后台重试） |
| `StatusDisconnected` | 已主动断开或未启动 |
| `StatusNone` | 组件无连接概念，或尚未产生任何连接活动 |

#### 两种对接方式

组件按是否复用 `base.SharedNode`，对接方式不同：

**方式一：使用 `SharedNode` 的组件（推荐）**

凡是嵌入 `base.SharedNode[T]` 的组件**自动实现**了状态查询接口，无需额外编码。组件只需在断线/重连的时机调用 `SetStatus` 同步状态：

```go
type MyClientNode struct {
    base.SharedNode[*my.Client] // 自动获得 ConnectionStatus()
}

// 断线（重连由内部心跳/读循环驱动）
func (x *MyClientNode) onDisconnect(reason error) {
    x.SharedNode.SetStatus(types.StatusReconnecting, reason.Error())
}

// 重连成功
func (x *MyClientNode) onReconnect() {
    x.SharedNode.SetStatus(types.StatusConnected, "")
}
```

> `SetStatus` 内部用独立的状态锁，**不会**与连接生命周期主锁竞争，因此可以在 `InitInstanceFunc` 回调（运行于 `GetSafely` 持锁期间）中安全调用，不会自死锁。

如果底层客户端库（如 paho MQTT）自身维护了更准确的实时连接状态，可以额外重写 `ConnectionStatus()` 以库的实时状态为准：

```go
func (x *MyClientNode) ConnectionStatus() types.StatusInfo {
    if client, ok := x.SharedNode.Instance(); ok { // Instance() 不触发懒初始化
    ...
    return x.SharedNode.ConnectionStatus() // 回退到 SharedNode 的快照
}
```

**方式二：不使用 `SharedNode` 的组件**

直接实现 `ConnectionStatusGetter` 接口即可，状态查询会被聚合层自动收录：

```go
type MyCustomNode struct {
    // 自行管理连接，未嵌入 SharedNode
    conn net.Conn
    mu   sync.RWMutex
...
func (x *MyCustomNode) ConnectionStatus() types.StatusInfo {
    x.mu.RLock()
    defer x.mu.RUnlock()
    return types.StatusInfo{Status: x.status, Message: x.statusMsg}
}
```

#### 初始化：严格模式与软失败模式

连接型组件在 `Init` 时通常通过 `SharedNode.InitWithClose` 建连，有两种失败语义：

- **`InitWithClose`（严格，默认）**：`initNow=true` 时建连失败原样返回 error，调用方可据此让"连不上即启动失败"。适合节点组件（数据库、工业协议等），由 `NodeClientInitNow` 配置控制。
- **`InitWithCloseSoftFail`（软失败）**：`initNow=true` 时建连失败不返回错误，仅置状态为 `Reconnecting`，由后续 `GetSafely` 在冷却窗口后懒重试。适合 endpoint——依赖服务（如 MQTT broker）可能晚于本服务就绪。

```go
// 节点组件：严格模式，遵循 NodeClientInitNow
if err := x.SharedNode.InitWithClose(ruleConfig, x.Type(), x.Config.Server,
    ruleConfig.NodeClientInitNow, x.initClient, x.closeClient); err != nil {
    return err
}

// endpoint：软失败，broker 未就绪也不阻断启动
_ = x.SharedNode.InitWithCloseSoftFail(ruleConfig, x.Type(), x.Config.Server,
    true, x.initClient, x.closeClient)
```

#### 状态聚合与查询

实现了 `ConnectionStatusGetter` 的组件，其状态会被自动聚合到规则链级别，无需组件自身关心：

```go
// 链内所有连接型组件的状态聚合（节点 + metadata 声明的 endpoint）
// types.ChainStatusesGetter 是 ChainCtx 的可选能力接口，按需断言
if g, ok := chainCtx.(types.ChainStatusesGetter); ok {
    statuses := g.Statuses() // map[组件id]StatusInfo
}
```

`rulego-server` 暴露了 HTTP 接口查询：

- `GET /rules/:id/status` —— 规则链内节点 + endpoint 的连接状态聚合
- `GET /shared-nodes/status` —— 共享节点池内所有节点的连接状态
...
在包中定义 `init()` 函数，导入包时自动完成注册，适合组件库场景：
...
使用 `Unregister` 卸载已有组件，再用 `Register` 注册新组件，可以替换内置组件的实现：

```go
func init() {
    // 先卸载内置的 restApiCall 组件
...
> 组件类型（`Type()` 返回值）相同即可替换，规则链配置无需修改。
...
组件的可视化配置（表单定义、Tag 约定、共享节点池等）参见 [组件可视化配置](/pages/custom-component-visual/)。
