...
- `global`变量: 访问[config.Properties](/pages/config/#Properties)的值，例如：`${global.key}`
- `vars`变量: 访问[规则链vars](/pages/rule-chain/#rulechain-configuration-%E8%A7%84%E5%88%99%E9%93%BE%E7%9A%84%E9%85%8D%E7%BD%AE%E4%BF%A1%E6%81%AF)值，例如：`${vars.key}`

> **全局变量：** 所有组件配置字段都支持使用。

:::tip
在 `switch`/`inclusive` 的 `case`、`while` 的 `condition`、`for` 的 `range` 等表达式字段中使用变量时，不要写成 `msg.x == ${vars.order_code}`，应写成 `msg.x == vars.order_code` 或 `${msg.x == vars.order_code}`。
:::
...
> **注意：** 跨节点变量只能在支持运行时变量替换的组件配置字段中使用，例如 `mqttClient` 的 `topic`、`restApiCall` 的 `url`、`dbClient` 的 `sql`、`functions` 的 `functionName` 等字段。`jsTransform`/`jsFilter`/`log` 等节点的 `jsScript` 字段**不支持** `${...}` 运行时变量替换。
