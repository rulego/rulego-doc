...
日志记录接口，默认使用：`DefaultLogger()`。[log组件](/pages/log/) 使用该记录器。
...
[AOP](/pages/aop-overview/) 切面列表。示例：
```go
//添加组件故障降级切面
config := rulego.NewConfig(
...
### js脚本中操作缓存
可以在js脚本相关组件例如：[js脚本转换器](/pages/js-transform/) [js脚本过滤器](/pages/js-filter/) ，通过`$ctx`内置变量中获取缓存对象，并调用对应的方法进行缓存操作。
```javascript
...
```
### Lua脚本中操作缓存
可以在Lua脚本相关组件例如：[lua脚本转换器](/pages/lua-transform/) [lua脚本过滤器](/pages/lua-filter/) ，
...
```
### 组件中操作缓存
可以通过组件操作缓存，参考:[缓存获取组件](/pages/cache-get/)、[缓存删除组件](/pages/cache-delete/)、[缓存设置组件](/pages/cache-set/)
