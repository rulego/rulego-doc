...
StreamSQL is used inside a rule chain as a node component: use `x/streamAggregator` for aggregation, `x/streamTransform` for non-aggregate streaming transforms.

```go
// 1. Define the rule chain DSL: StreamSQL aggregation node + result push node
ruleChain := `{"ruleChain":{"id":"streamsql_demo","name":"StreamSQL Aggregation Demo","root":true},"metadata":{"nodes":[{"id":"s1","type":"x/streamAggregator","name":"Stream Aggregation","configuration":{"sql":"SELECT device_id, AVG(temperature) as avg_temp FROM stream GROUP BY device_id, TumblingWindow('5m')"}},{"id":"s2","type":"restApiCall","name":"Push Result","configuration":{"restEndpointUrlPattern":"http://alert-service/api/alerts","requestMethod":"POST"}}],"connections":[{"fromId":"s1","toId":"s2","type":"Success"}]}}`

// 2. Create the rule engine instance (the ID argument must match ruleChain.id in the DSL)
engine, err := rulego.New("streamsql_demo", []byte(ruleChain))

// 3. Feed device data into the chain; the StreamSQL node aggregates over the 5-minute window and then triggers the push
msg := types.NewMsg(0, "deviceData", types.JSON, types.NewMetadata(), `{"device_id": "dev1", "temperature": 25.5}`)
engine.OnMsg(msg)
```
