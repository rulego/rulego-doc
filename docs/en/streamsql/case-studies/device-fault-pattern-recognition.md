...
Each scenario below shows only SQL + input + output; for how to run them see [Case Studies · How to Run Case SQL](/en/pages/streamsql-cases-overview/#how-to-run-case-sql). **Pattern-matching queries must use `Emit` + `AddSink`** (`EmitSync` does not support MATCH_RECOGNIZE); use `AddSyncSink` to preserve order.
:::
...
- [Pattern Matching (MATCH_RECOGNIZE)](/en/pages/streamsql-cep/) — full syntax reference (PATTERN / DEFINE / MEASURES / navigation / aggregates / SUBSET / FINAL-RUNNING / SKIP)
- [How to Run Case SQL](/en/pages/streamsql-cases-overview/#how-to-run-case-sql) — the unified Go boilerplate
- [Analytic functions](/en/pages/streamsql-analytical-functions/) — adjacent-event change detection (`lag`/`had_changed`), complementary to CEP
