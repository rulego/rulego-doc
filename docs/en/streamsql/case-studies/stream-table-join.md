...
::: tip How to run
A JOIN query is a non-aggregation query; use `EmitSync` for synchronous results. Note that you must call `RegisterTable` after `Execute` to register the metadata table. See [How to Run Case SQL](/en/pages/streamsql-cases-overview/#how-to-run-case-sql) for the full boilerplate.
:::
