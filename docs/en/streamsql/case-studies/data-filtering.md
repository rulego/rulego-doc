...
- **`WHERE` filters raw rows**: rows that fail the condition return `nil`, so you can distinguish "filtered" from "error". To filter after aggregation, use `HAVING` (see [Sliding Window and Continuous Detection](/en/pages/streamsql-case-sliding/)).
...
::: tip How to run
Non-aggregation queries use `EmitSync(data)` for a synchronous single-row result; no `AddSink` needed. See [How to Run Case SQL](/en/pages/streamsql-cases-overview/#how-to-run-case-sql) for the full boilerplate.
:::
...
- [IoT Temperature Alerting and Metrics Aggregation](/en/pages/streamsql-case-iot-temperature/) — upgrading from single-point transform to windowed aggregation
