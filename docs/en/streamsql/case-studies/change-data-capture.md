...
In IoT, "change data capture" means **identifying events that actually changed from a continuous device-report stream** — a metric jumping across a threshold, a status flipping, a field changing, etc. StreamSQL's [analytic functions](/en/pages/streamsql-analytical-functions/) (`lag` / `had_changed` / `changed_col` / `changed_cols` / `latest` / `acc_*`) are designed precisely for this per-event state computation: each datum is evaluated on arrival, state persists across events, and they can be used in both `SELECT` and `WHERE`.
...
- [Analytic Functions](/en/pages/streamsql-analytical-functions/) - Full analytic-function syntax and function list
- [SQL Reference](/pages/streamsql-sql/) - HAVING, window functions, and the full SQL syntax
- [IoT Temperature Alerting and Metrics Aggregation](/en/pages/streamsql-case-iot-temperature/) - Windowed-aggregation IoT cases
