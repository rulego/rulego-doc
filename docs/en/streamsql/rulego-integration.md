...
functions.RegisterCustomFunction(
    ...
    func(ctx *functions.FunctionContext, args []interface{}) (interface{}, error) {
        temp, _ := cast.ToFloat64E(args[0])
        humidity, _ := cast.ToFloat64E(args[1])
...
## IoT Protocol Acquisition Scenario

When the data source is industrial devices (PLCs, meters, etc.) instead of MQTT, use [rulego-components-iot](/en/pages/iot-overview/) for protocol acquisition (`x/iotRead`, supporting 9 protocols including Modbus/S7/OPC UA/FINS). The acquisition point array connects **directly** to `x/streamAggregator` (`GROUP BY name` for per-point stats; set `inputFormat: columns` to pivot into a wide row for cross-point calculations), and `x/tsdbWrite` stores the aggregated results:

```
x/iotRead → x/streamAggregator → x/tsdbWrite
```

Full rule chain DSL: [IoT Scenario Examples · Acquisition → Aggregation → Store](/en/pages/iot-scenarios/).