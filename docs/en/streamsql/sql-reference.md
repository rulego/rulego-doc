...
| MATCH_RECOGNIZE | No | Pattern matching (CEP), recognizing event sequences — see [Pattern Matching](/en/pages/streamsql-cep/) |
...
## MATCH_RECOGNIZE Clause (Pattern Matching / CEP)

`MATCH_RECOGNIZE` recognizes **event sequences that appear in a specific order** — consecutive threshold crossings, rise-then-drop, start/stop workflows, out-of-order events, time-windowed sequences. SQL:2016 standard, Flink-aligned, written after `FROM stream`:

```
SELECT * FROM stream
MATCH_RECOGNIZE (
    [PARTITION BY <expr> [, ...]]              -- match independently per partition
    ORDER BY <expr> [ASC]                      -- required: event ordering (usually a timestamp)
    [MEASURES <expr> AS <alias> [, ...]]       -- output columns
    [ONE ROW PER MATCH | ALL ROWS PER MATCH]   -- default ONE ROW
    [AFTER MATCH SKIP ...]                     -- default PAST LAST ROW
    PATTERN ( <pattern> )                      -- sequence/quantifiers ?*+{n} / alternation | / group / PERMUTE
    [SUBSET <name> = (<sym> [, ...]) [, ...]]
    [WITHIN '<interval>']                      -- time window (recommended)
    DEFINE <sym> AS <cond> [, ...]             -- symbol conditions; undefined symbols are always true
)
```

| Clause | Required | Description |
|--------|----------|-------------|
| `PARTITION BY` | no | Match independently per partition (e.g. per device) |
| `ORDER BY` | **yes** | Events processed ascending by this field |
| `MEASURES` | no | Match output columns; if omitted, emits the match's last-row fields |
| `PATTERN` | **yes** | Pattern expression: quantifiers `? * + {n} {n,} {n,m}`, alternation `\|`, grouping `()`, `PERMUTE` |
| `DEFINE` | no | Per-symbol conditions; may use `PREV`/`NEXT`/`FIRST`/`LAST`/aggregates |
| `WITHIN` | no (recommended) | Time window; idle partitions' out-of-window partial matches are actively expired |

`MEASURES`/`DEFINE` may use row-pattern functions: `PREV`/`NEXT`/`FIRST`/`LAST` (navigation), `SUM`/`AVG`/`COUNT`/`MIN`/`MAX` (aggregates, symbol-scoped like `SUM(A.v)`), `CLASSIFIER()` (the symbol of the current row), `MATCH_NUMBER()` (the match index).

```sql
-- Three consecutive temperature threshold crossings (debounce)
SELECT * FROM stream
MATCH_RECOGNIZE (
...
::: warning Key points of CEP
- **Must use `Emit` + `AddSink`**: `EmitSync` does not support MATCH_RECOGNIZE (many-in/many-out semantics); calling `EmitSync` returns an error.
- **SELECT does not project**: output is decided by `MEASURES`; the outer is always `SELECT * FROM stream MATCH_RECOGNIZE(...)`.
- **`ORDER BY` is required**: pattern matching depends on event order; omitting it is an error.
- **Greedy quantifiers** (`A*`/`A+`) with no natural endpoint emit only when the run terminates (next non-matching row) or on `Stop` via `Flush`.
:::

Full syntax (`SUBSET`, `FINAL`/`RUNNING`, all `AFTER MATCH SKIP` strategies, greedy/reluctant, the four bounded-memory guards, known limitations) is in [Pattern Matching](/en/pages/streamsql-cep/); business scenarios are in [Device Fault Pattern Recognition](/en/pages/streamsql-case-cep/).
...
Analytic functions perform cross-event state computation on a continuous event stream (change detection, context backtracking, accumulation); each event is evaluated on arrival and they are usable in `SELECT` and `WHERE`. See [Analytic Functions](/en/pages/streamsql-analytical-functions/).
