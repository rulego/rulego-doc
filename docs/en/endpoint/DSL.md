...
// Return true to execute the next processor or the To operation
// Return false to abort and do not proceed further
return true
})
```
Then reference it by `name` in the DSL: `InBuiltins` processors go into `from.processors`, `OutBuiltins` processors go into `to.processors`. Multiple can be specified, executed in configuration order:
```json
"from": {
  "path": "/api/v1/test/:chainId",
  "processors": ["headersToMetadata"]
}
```
