...
The binlog is a single-point stream: in a multi-replica deployment every replica reads it in full and triggers the rule chain repeatedly. Once the rule chain config injects a [distributed lock](/pages/locker/), this endpoint elects a leader automatically — only the leader reads the binlog, and a standby takes over when the leader is lost, resuming from the latest position (or the `fromOldest` setting) without replaying the gap — the same semantics as a crashed process restarting. Single-process deployments are unaffected.
