...
Common configs:
- `${msg.deviceId}`: deviceId field of a JSON first frame
- `${msg.header.sn}`: nested field
- `${hex(data[4:14])}`: binary protocol, hex of bytes at offset 4-14 as the key
- `${reFind("ID:([a-zA-Z0-9_]+)", data)}`: regex extraction (**must use double quotes**, el engine doesn't accept single quotes; avoid `\w`, use `[a-zA-Z0-9_]`)
- empty: defaults to the device RemoteAddr (IP:Port)

### Active push

After the device connects and the sessionKey is extracted, use a `net` node (`server=ref://<this endpoint instance ID>`, `target=deviceId or *`) to push to a specific device or all devices. See [net component - Session Addressing Push](/pages/net/).
