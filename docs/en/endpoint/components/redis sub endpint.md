...
Pub/Sub is a broadcast model: in a multi-replica deployment every message triggers every replica. Once the rule chain config injects a [distributed lock](/pages/locker/), this endpoint elects a single active consumer automatically — only the leader subscribes, and a standby takes over when the leader is lost. Single-process deployments are unaffected.
