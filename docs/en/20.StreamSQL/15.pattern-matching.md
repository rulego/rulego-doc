...
::: warning Same keyword as the stream-stream JOIN, different meaning
`WITHIN` in MATCH_RECOGNIZE bounds the **active lifetime of a pattern sequence**; `JOIN ... WITHIN 30 SECONDS` ([Stream-Stream JOIN](/pages/streamsql-stream-join/)) bounds the **timestamp distance between two rows**. The two never coexist in one query.
:::
