...
- Analytic functions **cannot be mixed with `GROUP BY`/windows** and cannot appear in `HAVING`; for time-windowed / sustained detection use windowed aggregation + `HAVING` (see [Sliding Window & Sustained Detection](./31.case-studies/04.sliding-window.md)).
