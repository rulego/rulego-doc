...
- 🔗 **Stream-Stream JOIN**: `WITHIN` time-proximity correlation of two live streams — dual-source signal corroboration, command-ACK absence alerting (LEFT NULL complement), event-time alignment, 3+ stream left-deep cascades. Aligned with Flink Interval Join / ksqlDB / Kafka Streams JoinWindows; bounded state suits the edge. See [Stream-Stream JOIN](/pages/streamsql-stream-join/).
...
| **Stream-Stream JOIN** | ✅ WITHIN time-proximity | ✅ Interval Join | ❌ None | ✅ WITHIN | ❌ None | ❌ None |
...
- 🔗 [Stream-Stream JOIN](/pages/streamsql-stream-join/) - dual-stream time-proximity correlation and absence alerting
