...
- 🧩 **Pattern Matching (CEP)**: `MATCH_RECOGNIZE` recognizes event-sequence patterns — consecutive threshold crossings, rise-then-drop, start/stop workflows, out-of-order events, time-windowed sequences. SQL:2016 standard syntax (Flink-aligned); `PATTERN`/`DEFINE`/`MEASURES`/navigation/aggregates/`SUBSET`/`FINAL`/`WITHIN` all supported, with four guards bounding edge memory. See [Pattern Matching](/pages/streamsql-cep/).
