...
::: warning Limitation
Stream-table JOIN is for enrichment only and runs on the non-window path. **Stream-table JOIN combined with aggregation/window is rejected** (`Execute` returns `"JOIN with aggregation/window is not supported"`). To use windowed aggregation, remove the JOIN. (Since v1.3.0 there is also a `WITHIN` stream-stream JOIN — see the next section, which is not subject to this.)
:::
...
## Stream-Stream JOIN API

The stream-stream JOIN (v1.3.0+) is declared in SQL with `WITHIN` (syntax and semantics: [Stream-Stream JOIN](./16.stream-stream-join.md)); on the Go side, data is fed by stream name through the APIs below. All are new additive APIs; existing red-line APIs are unchanged.
...
Delivers one row to the named input stream.

- `stream` — the stream name as written after `FROM` / `JOIN` in the SQL, **case-sensitive**;
- An unknown name returns an error (the message lists known stream names);
- `Emit(row)` is equivalent to `EmitTo(the FROM stream, row)`, so single-stream code needs no change;
- Available only for stream-stream JOIN (WITHIN) queries; on other queries it returns an error (hinting to use `Emit`).
...
Reports whether the current query is a stream-stream JOIN (call after `Execute`). Lets multi-source integration layers — such as RuleGo components — route rows by the stream name in metadata via `EmitTo`.

### WithJoinMaxKeys / WithJoinMaxRows

```go
func WithJoinMaxKeys(n int) Option  // buffered key cap per side per stage (LRU, default 10000)
func WithJoinMaxRows(n int) Option  // buffered row cap per side per stage (default 0 = unbounded)
```
...
ssql.AddSink(func(rows []map[string]interface{}) { /* alert rows */ })

_ = ssql.EmitTo("cmdStream", map[string]interface{}{"cmdId": "c1", "deviceId": "d1"})
_ = ssql.EmitTo("ackStream", map[string]interface{}{"cmdId": "c1", "ackCode": 0})

ssql.Stop() // unmatched LEFT pending rows are NULL-complemented before exit
```

::: warning
`EmitSync` returns an error on stream-stream JOIN queries (same as CEP queries); results can only be consumed asynchronously via `AddSink` / `ToChannel`.
:::
...
- [Performance Optimization](https://github.com/rulego/streamsql/blob/main/docs/PERFORMANCE_GATEWAY_CAPACITY.md) - Learn about performance optimization techniques
- [Custom Functions](https://github.com/rulego/streamsql) - Learn how to develop custom functions
