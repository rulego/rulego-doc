...
| JOIN | No | Stream-table JOIN (metadata enrichment) or stream-stream JOIN (`WITHIN`, time-proximity correlation) — see [JOIN Clause](#join-clause) |
| MATCH_RECOGNIZE | No | Pattern matching (CEP), recognizing event sequences — see [Pattern Matching](./15.pattern-matching.md) |
...
JOIN comes in two forms, **distinguished by the presence of `WITHIN`**:

**① Stream-table JOIN (metadata enrichment, no WITHIN)** — stream × registered table (`RegisterTable`), looked up against the current table snapshot:

```sql
SELECT deviceId, m.location, temperature
FROM stream JOIN meta m ON deviceId = m.deviceId
```

**② Stream-stream JOIN (time-proximity correlation, WITHIN required, v1.3.0+)** — two live streams joined by key with `|L.ts − R.ts| ≤ WITHIN`, aligned with Flink Interval Join / ksqlDB / Kafka Streams JoinWindows:
...
Stream-stream JOIN essentials:

- `WITHIN 30 SECONDS` / `WITHIN '30s'` / `WITHIN 100 MS` are equivalent; positioned after the JOIN table name or after ON (exactly one);
- JOIN types: INNER / LEFT (unmatched LEFT rows are NULL-complemented at window close); RIGHT/FULL/CROSS are rejected;
- Event time: `WITH (TIMESTAMP = 'ts')`; arrival time by default;
- ON accepts equality conditions only, composite keys chained with AND; 3+ streams form a left-deep cascade with per-stage WITHIN;
- Not supported: WITHIN JOIN combined with GROUP BY / windows / HAVING / DISTINCT / LIMIT / analytic functions / stream-table JOIN / unnest (compile-time errors).

Full semantics, resource bounds and more examples: [Stream-Stream JOIN](./16.stream-stream-join.md).
...
Full syntax (`SUBSET`, `FINAL`/`RUNNING`, all `AFTER MATCH SKIP` strategies, greedy/reluctant, the four bounded-memory guards, known limitations) is in [Pattern Matching](./15.pattern-matching.md); business scenarios are in [Device Fault Pattern Recognition](./31.case-studies/07.device-fault-pattern-recognition.md).
...
- `UNION` operations (result merging)
...
- Triggers (TRIGGER)
- `RIGHT/FULL/CROSS JOIN` (JOIN supports two forms: [stream-table JOIN](#join-clause) and `WITHIN` stream-stream JOIN)

### Restriction Notes

1. **Data sources**: a single-stream query has a single data source; multiple sources are only available in the JOIN forms (a stream-stream JOIN is fed by stream name)
2. **No persistence**: data persistence is not supported
3. **No transactions**: transactions are not supported
4. **Memory limit**: bounded by single-machine memory (stream-stream JOIN state is bounded by the WITHIN retention period plus LRU/row-count gates)
