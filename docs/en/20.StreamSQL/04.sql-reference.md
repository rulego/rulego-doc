...
Analytic functions take an `OVER ([PARTITION BY ...] [WHEN ...])` clause to control partitioning and conditional state. **They cannot be mixed with `GROUP BY`/windows, nor appear in `HAVING`**; for time-windowed / sustained detection use windowed aggregation + `HAVING`.
