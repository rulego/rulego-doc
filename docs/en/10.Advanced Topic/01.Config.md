...
[AOP](/pages/aop-overview/) aspect list. Example:
```go
...
In Lua script-related components, such as [Lua Script Transformer](/pages/lua-transform/) and [Lua Script Filter](/pages/lua-filter/), you can obtain the cache object through the built-in variables `ChainCache` or `GlobalCache` and call the corresponding methods to operate the cache.
