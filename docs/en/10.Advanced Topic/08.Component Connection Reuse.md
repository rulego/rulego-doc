...
Applicable components: `endpoint/net` (TCP/UDP), `endpoint/ws` (WebSocket). These endpoints have a built-in session registry that registers a session when a device connects; a `net`/`ws` node configured with `server=ref://<endpoint instance ID>` can reuse these sessions to actively push data to specific devices by `target` (sessionKey identifier, e.g. deviceId; `*` broadcast), exact match.
...
3. The node looks up the target device's connection from the session pool and reuses it to push
