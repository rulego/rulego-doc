...
- `script`: This can transform `msg`, `metadata`, and `msgType`. Only the function body content is required. If it is a file path, a complete script function must be provided:
  ...
  ```
  - msg: Message content. If [dataType=JSON](/en/pages/rule-chain-message/), it can be accessed and modified using `msg.temperature`. If dataType is of another type, the field type is `string`.
