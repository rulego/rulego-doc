...
Filtered rows (WHERE not matched, or `changed_cols`/`changed_col` unchanged) now route to the `False` chain instead of `Failure` (filtering is not an error). If your DSL wired the filtered output to the Failure chain, reconnect it to the False chain; Failure is reserved for genuine processing errors.
:::
...
### Single Data Input
Directly process a single JSON object: successful transformation goes to Success; WHERE not matched goes to False; a processing error goes to Failure:
```json
...
- If all elements are filtered by WHERE (no errors), the merged result goes to False; if any element errors, it goes to Failure.
...
The `inputFormat` config decides how it is processed - **the two modes produce different output structures**:

| inputFormat | Processing | Output structure | Suited for |
|---|---|---|---|
| `auto` (default) | each point is transformed row by row (columns `name/value/timestamp`), results merged into an **array** | `[{"name":"temperature","value":25.3,...}, ...]` | per-point transform/change detection; when the output keeps `name/value` keys, `x/tsdbWrite` pivots it into one record |
| `columns` | the whole batch is pivoted into one wide row `{pointName:value}` (bad points skipped), processed as a single record | **a single flat map**: `{"temperature":25.3,...}` | cross-point calculations; the whole output map becomes the fields of one `x/tsdbWrite` record |

Long-format example (per-point change detection):
...
| inputFormat | string | Array input mode: `auto` transforms each element and merges into an array; `columns` pivots an IoT point array into one wide row and outputs a single flat map | auto |
...
- **False:** Data was filtered (WHERE not matched, or `changed_cols`/`changed_col` had no change) — **not an error**; metadata `match=false`.
...
Being filtered by WHERE or having no `changed_cols` change is **an intentional no-output**, routed to **False**. Only genuine processing errors go to **Failure**. So the Failure chain can safely be wired to alerts/error handling without being polluted by normal event compression.
:::
...
### False Chain Output
The original (untransformed) input data: when WHERE is not matched or `changed_cols`/`changed_col` has no change, the message body is unchanged — still the original incoming data (metadata `match=false`), so downstream can inspect/log what was filtered.
...
::: tip "No change" goes to the False chain
When `changed_cols`/`changed_col` is the sole output and nothing changed this time, the node returns nil → routed to the **False** chain (`match=false`). This is the intended event compression — wire the False chain to drop/ignore; it never pollutes the Failure chain.
:::
...
{temperature:23, humidity:55} → goes to the False chain (nothing changed, row suppressed)
```

::: tip No change goes to the False chain
When `changed_cols` is the sole output and nothing changed, it returns nil → routed to the **False** chain (`match=false`). This is the intended event compression — wire the False chain to drop/ignore.
:::
...
5. **WHERE Condition:** Data that does not meet the WHERE condition is filtered out (routed to the False chain) and excluded from the output.
