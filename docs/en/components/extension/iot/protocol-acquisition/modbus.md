...
| address            | string | Register address, placeholder variables `${}` are allowed. Format: uint16， Example: 50 or 0x32                                                                                             | None                  |
...
| certFile           | string | Path to the certificate (required for TLS/SSL connections)                                                                                                                                 | None                  |
| certKeyFile            | string | Path to the private key (required for TLS/SSL connections)                                                                                                                                 | None                  |
| caFile             | string | Path to the CA certificate (required for TLS/SSL connections)                                                                                                                              | None                  |
...
| parity             | string | Parity mode (RTU mode only), options:<br>`N` - None<br>`E` - Even<br>`O` - Odd<br><br>Full words `none`/`even`/`odd` also work, case-insensitive.<br>For compatibility with earlier configs, the numbers `0`/`1`/`2` are still parsed (mapping to N/E/O in that order), but exports and docs use the letter.<br><br>Note: letters match how devices are documented in the field (e.g. `8E1`). The RTU driver has no mark/space parity — passing `M`/`S` fails with a clear error rather than silently falling back to none; use the [x/serial](/en/pages/x-serial/) component when you need those. | `N`                   |
