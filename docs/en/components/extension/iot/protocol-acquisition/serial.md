...
1. **Periodic data collection**:
   - `endpoint/schedule` (timer) -> `x/serialIn` (read) -> `Log` (print)
2. **Device control**:
   - `endpoint/mqtt` (receive command) -> `x/serialOut` (send command)
3. **Query device status**:
   - `endpoint/http` (receive request) -> `x/serialRequest` (query and wait) -> `Http Response` (return result)
