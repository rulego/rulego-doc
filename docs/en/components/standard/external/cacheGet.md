...
| whenKeyNotFound | string     | Strategy when key is not found (case-insensitive) | Empty    |
...
- **whenKeyNotFound**: Strategy when key is not found, case-insensitive
  - Empty or unset: Default behavior (Mode 0/1 go to Success, Mode 2 goes to Failure on all miss)
  - failure: Send to Failure chain when key is not found
  - success: Send to Success chain when key is not found
### LevelKey

| Field | Type   | Description                                                               | Default Value |
|-------|--------|---------------------------------------------------------------------------|---------------|
| level | string | Cache level                                                               | chain         |
| key   | string | Key, which can use [component configuration variables](/en/pages/component-configuration-variables/) | None          |
...
- ***Failure:*** If the retrieval fails (including underlying cache client errors) or the cache is not found according to the `whenKeyNotFound` configuration, the message is sent to the `Failure` chain.
