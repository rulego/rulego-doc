...
> JavaScript scripts support ECMAScript 5.1(+) syntax specifications and some ES6 features, such as async/await/Promise/let. Custom Go functions can be called within the script. Please refer to [udf](/pages/config/#udf).
...
- `jsScript`: The JavaScript script used to write filtering logic. The content of this field will be encapsulated within the following function body:
  ...
  - msg: Message content
    - When [dataType=JSON](/en/pages/rule-chain-message/), the type is `jsonObject`, and properties can be accessed directly using dot notation, such as `msg.temperature`.
...
2. Script execution has a timeout limit, which can be configured through [config.ScriptMaxExecutionTime](/pages/config/#ScriptMaxExecutionTime).
   :::
