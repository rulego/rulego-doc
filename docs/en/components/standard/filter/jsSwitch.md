...
> JavaScript scripts support ECMAScript 5.1(+) syntax specifications and some ES6 features, such as async/await/Promise/let. Custom Go functions can be called within the script. For more details, refer to [udf](/pages/config/#udf).
...
- `jsScript`: This field can process and evaluate `msg`, `metadata`, `msgType`, and `dataType`. It is the body of the following function:
  ...
  - msg: Message content
    - When [dataType=JSON](/en/pages/rule-chain-message/), it is of type `jsonObject`, and fields can be accessed using `msg.temperature`.
...
::: danger Note
1. The script execution timeout configuration can be found at [config.ScriptMaxExecutionTime](/pages/config/#ScriptMaxExecutionTime).
