...
|------------------|-------------------|-----------------------------------------------------|---------------|
| RuleChain        | RuleChain         | Rule chain snapshot: [Reference](/en/pages/rule-chain/) | None          |
...
| inMsg        | RuleMsg  | Input message: [Reference](/en/pages/rule-chain-message/)  | None          |
| outMsg       | RuleMsg  | Output message: [Reference](/en/pages/rule-chain-message/) | None          |
...
Options for the engine instance (`types.NewConfig`):

#### WithLocker <Badge text="v0.38.0+"/>

Inject a distributed lock to enable cross-replica dedup (at-most-once scheduled triggers) and single-active election for broadcast endpoints (Redis Pub/Sub, RabbitMQ, MySQL CDC). Redis implementation:

```go
import "github.com/rulego/rulego-components/pkg/locker"

config := types.NewConfig(types.WithLocker(locker.NewRedisLocker(redisClient)))
```

See [At-Most-Once Execution](/pages/locker/).

#### WithOwner <Badge text="v0.38.0+"/>

Set the engine owner identity — the tenant partition name in multi-tenant deployments:

```go
types.WithOwner("tenant1")
```
