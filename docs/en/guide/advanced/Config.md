...
[AOP](/en/pages/aop-overview/) aspect list. Example:
```go
...
In Lua script-related components, such as [Lua Script Transformer](/en/pages/lua-transform/) and [Lua Script Filter](/en/pages/lua-filter/), you can obtain the cache object through the built-in variables `ChainCache` or `GlobalCache` and call the corresponding methods to operate the cache.
...
Distributed lock. Inject it in multi-replica deployments to enable two cross-replica semantics:

- **Message-level dedup** (`OnceGuard`): in-process trigger sources run at most once per cluster — for example, the [schedule endpoint](/pages/endpoint-schedule/) executes each scheduled slot exactly once
- **Single-active election** (`ActiveGuard`): broadcast endpoints are consumed only by the leader replica, which a standby replaces automatically when the leader is lost. Built into the Redis Pub/Sub, RabbitMQ and MySQL CDC endpoints

When nil, it has no effect and the behavior is identical to a single process deployment. The backend is not fixed; Redis is the usual choice: [rulego-components/pkg/locker](https://github.com/rulego/rulego-components/blob/main/pkg/locker/redis_locker.go).

See [At-Most-Once Execution](/pages/locker/).

## Owner <Badge text="v0.38.0+"/>

Type: `string`

The identity of the engine instance — the tenant partition name in multi-tenant deployments. It namespaces resources that must not be shared across engines: lock keys, cache keys, message broker subscription groups. It is a different concept from the tenant of a particular message, which travels in message metadata.
