...
permalink: /pages/custom-components-overview/
---
...
Define an `init()` function in the package to automatically register components when the package is imported. This is suitable for component libraries:
...
func init() {
    _ = rulego.Registry.Register(&MyNode{})
}
```

Use with anonymous import:

```go
import _ "github.com/example/mycomponents"
```

### Replacing and Unregistering Components

Use `Unregister` to remove an existing component, then `Register` to register a new one. This allows replacing built-in component implementations:

```go
func init() {
    // Unregister the built-in restApiCall component
    _ = rulego.Registry.Unregister("restApiCall")
    // Register a custom implementation to replace it
    _ = rulego.Registry.Register(&RestApiCallNode{})
}
```

> Components with the same type (the return value of `Type()`) can be replaced without modifying the rule chain configuration.
...
For component visual configuration (form definitions, tag conventions, shared node pool, etc.), see [Component Visual Configuration](/en/pages/custom-component-visual/).
