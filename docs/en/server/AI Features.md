...
For complete documentation on AI extension components (agent framework architecture, `ai/agent` node configuration, tool system, aspect framework, session management, LLM/intent recognition/MCP component references), see [AI Extension Components](/en/pages/ai-agent-overview/).
...
> Prompt files are stored in `data/system/agents/{agentId}/AGENTS.md`. Manage prompts and model configuration via API; see [REST API Reference](/en/pages/rulego-server-api/).

## Skill Management

Skills are knowledge fragments provided to AI Agents, stored as `SKILL.md` files, enabling the Agent to understand domain-specific knowledge. They can be managed through the editor or API; see [REST API Reference](/en/pages/rulego-server-api/).
...
For client configuration, available tool list, and usage examples, see [MCP Service](/en/pages/rulego-server-mcp/).

## Using AI in the Editor

> For an illustrated guide, see [AI Assistant Tutorial](/en/pages/rulego-server-ai-tutorial/).
...
> For an illustrated tutorial, see [Creating an Agent Tutorial](/en/pages/rulego-server-create-agent/).

## AI Tool Security

In demo environments or scenarios where AI tool capabilities need to be restricted, you can configure a global tool interception policy via the `[ai_security]` section in `config.conf` (e.g. disabling the bash/write/edit tools, restricting file access paths, etc.), without affecting each agent node's own tool security settings. For configuration details, see [Installation and Deployment - AI Tool Security Configuration](/en/pages/rulego-server-install/#ai-tool-security-configuration).
