...
## Ecosystem: GFlow Approval Workflow

[GFlow Approval Workflow](https://gflow.rulego.cc/) is an enterprise approval workflow platform built on RuleGo: **approve, then automate** — AI reviews first, humans sign next, and actions execute automatically once signed. Approvals and automation live in the same process DSL: the `automation` node invokes rule chains to run business actions, and the `aiAgent` node invokes agents to assist approval, bringing RuleGo's orchestration capabilities straight into approval scenarios.
