...
```http
Authorization: Bearer {jwtTokenOrApiKey}
```
...
| Method | Path | Permission | Description |
|--------|------|------------|-------------|
| GET | `/api/v1/overview` | `rule:read` | Get overview statistics for the current user (rule chain counts, status distribution, etc.) |
| GET | `/api/v1/version` | Authenticated | Get server version information |

## User Management

| Method | Path | Permission | Description |
|--------|------|------------|-------------|
| GET | `/api/v1/users/me` | Authenticated | Get current logged-in user information |
| PATCH | `/api/v1/users/me` | Authenticated | Modify current user information |
| GET | `/api/v1/users` | `user:read` | List all users (admin) |
| POST | `/api/v1/users` | `user:write` | Create/update user (admin) |
| DELETE | `/api/v1/users/:targetUsername` | `user:delete` | Delete user (admin, `?purge=true` removes data) |

See [Authentication and Authorization](/en/pages/rulego-server-auth/) for details.
...
| GET | `/api/v1/category/rule` | `rule:read` | Get rule chain category tree |
| GET | `/api/v1/rules/:id` | `rule:read` | Get rule chain DSL |
| GET | `/api/v1/rules/:id/latest` | `rule:read` | Get the most recently modified rule chain for the current user (`:id` can be `_`) |
| GET | `/api/v1/rules/:id/status` | `rule:read` | Get rule chain runtime status |
...
Structure of the debug log data pushed by the server:

| Field | Description |
|-------|-------------|
| `chainId` | Rule chain ID |
| `flowType` | Flow direction: `In` (node receives data) / `Out` (node finished processing) |
| `nodeId` | Node ID |
| `relationType` | Relation type (Success, True, False, etc.) |
| `err` | Error message |
| `msg` | Message content |
...
ws.onmessage = (event) => {
  ...
  console.log(`[${data.flowType}] Node: ${data.nodeId}`, data.msg)
}
```
...
| GET | `/api/v1/shared-node-statuses` | `component:read` | Get shared node runtime statuses |
...
| Method | Path | Permission | Description |
|--------|------|------------|-------------|
| POST | `/api/v1/builtins/:name` | `config:read` | Execute a built-in processor (e.g. script pre-check, expression evaluation) |
...
| Method | Path | Permission | Description |
|--------|------|------------|-------------|
| GET | `/api/v1/iot/point-templates` | `config:read` | List point templates |
| GET | `/api/v1/iot/point-templates/:id` | `config:read` | Get point template |
| POST | `/api/v1/iot/point-templates` | `config:write` | Create point template |
| PUT | `/api/v1/iot/point-templates/:id` | `config:write` | Update point template |
| DELETE | `/api/v1/iot/point-templates/:id` | `config:write` | Delete point template |
