...
│   │   ├── mcp/         #   MCP module
│   │   └── iotpoint/    #   IoT point template module
...
| iotpoint | 75 | IoT point templates |
...
| `KeyUserAdmin` | `module.user.admin` | `UserAdmin` | User management (CRUD) |
...
| `KeyDebugService` | `module.debug.service` | `DebugService` | Debug service (registered by the runlog module) |
| `KeyIoTPointService` | `module.iotpoint.service` | `PointTemplateService` | IoT point templates |
...
type Authenticator interface {
    // Parses the authorization header (raw "Bearer xxx" value) and returns a UserContext; returns an error on failure
...
type Authorizer interface {
    // Checks whether user can perform action on resource; returns nil if allowed, otherwise an error
...
| `use_fasthttp` | Replaces the HTTP endpoint and restApiCall components with fasthttp implementations. Throughput improves by ~20%~30% for short-lived connections / high concurrency (p99 drops by ~40%), and by ~7%~10% for regular keep-alive APIs; choose by workload characteristics |

```bash
# Enable fasthttp endpoint (requires a fixed rulego-components version; CGO_ENABLED=0 static build recommended)
