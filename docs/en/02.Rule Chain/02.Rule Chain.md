...
Used to configure the automatic triggering of the rule chain, such as mqtt, http, ws, schedule, and other Endpoint triggers, detailed reference: [Endpoint](/pages/0d77d2/)

| Field    | Type     | Required | Description                          |
|----------|----------|----------|--------------------------------------|
| endpoint | object   | Yes      | Basic information of the Endpoint |
| routes   | object[] | Yes      | Routes                               |

#### metadata.endpoints.endpoint: Basic Information of the Endpoint
...
| configuration  | object   | No       | Endpoint configuration, different Endpoint types have different configuration fields, please refer to the specific Endpoint component configuration documentation. |
| additionalInfo | object   | No       | Extended field, used to save additional information.                                                                                                                     |
| processor      | string[] | No       | List of processors, interceptor names must be registered on the platform.                                                                                                |

#### metadata.endpoints.routes: List of Endpoint Routes

| Field          | Type     | Required | Description                                                                        |
|----------------|----------|----------|------------------------------------------------------------------------------------|
| id             | string   | Yes      | Route ID                                                                           |
| params         | string[] | No       | Route parameters, for example: HTTP Endpoint route parameters are: POST/GET/PUT... |
| from           | From     | Yes      | Source of the route                                                                |                                                 |
| to             | To       | No       | Target of the route                                                                |
| additionalInfo | object   | No       | Extended field, used to save additional information.                               |

#### metadata.endpoints.routes.from: Source of the Endpoint Route

| Field         | Type   | Required | Description                                                                                              |
|---------------|--------|----------|----------------------------------------------------------------------------------------------------------|
| path          | string | Yes      | Source path of the route, for example: http Endpoint /api/msg/:chainId  scheduler Endpoint */1 * * * * * |
| configuration | object | No       | Source configuration of the route, not required                                                          |
| processor     | From   | Yes      | List of processors, interceptor names must be registered on the platform.                                |                                                 |

#### metadata.endpoints.routes.to: Target of the Endpoint Route

| Field         | Type    | Required | Description                                                                                   |
|---------------|---------|----------|-----------------------------------------------------------------------------------------------|
| path          | string  | Yes      | Target path of the route, can be an executing rule chain ID or a variable such as: ${chainId} |
| configuration | object  | No       | Target configuration of the route, not required                                               |
