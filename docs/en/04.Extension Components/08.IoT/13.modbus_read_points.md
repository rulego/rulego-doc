...
permalink: /pages/x-modbus-read-points/
---

`x/modbusReadPoints` component: <Badge text="v0.35.0+"/> batch-reads Modbus points over TCP/RTU. Results are written back to `msg.Data` as a list of the **unified contract** `iot_points.Data`, routing to `Success`/`Failure`.
...
- Write: [x/modbusWritePoints](/pages/x-modbus-write-points/)
- Universal acquisition entry: [x/iotRead](/pages/x-iot-read/)
- To time-series: x/iotToSeries → [x/tsdbWrite](/pages/x-tsdb-write/)
- Imperative (legacy): [x/modbus](/pages/x-modbus/) (kept; output also unified to iot_points.Data)
