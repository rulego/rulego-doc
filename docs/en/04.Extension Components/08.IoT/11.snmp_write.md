...
The connection configuration is identical to [`x/snmpRead`](/pages/x-snmp-read/); all fields are listed below:
...
Fields of each Point (the write node uses `name`/`oid`/`type`/`value`; `op` is read-node only):

| Field | Type   | Description                                                       | Default |
|-------|--------|-------------------------------------------------------------------|---------|
| name  | string | Point name (optional)                                             | None    |
| oid   | string | SNMP OID to write, e.g. `1.3.6.1.2.1.1.5.0`                      | None    |
| type  | string | Value type, see the table below                                  | None    |
...
> `value` supports `${msg.xx}` / `${metadata.xx}` template variables. If any point fails to write, the node returns immediately and does not process subsequent points.

## Features

- **Template variables**: the `oid`/`type`/`value` fields support `${msg.xx}` / `${metadata.xx}` placeholders.
...
- **Same-chain connection pool**: enables `ref://` in-chain connection reuse; other nodes in the chain can borrow the connection by node ID to avoid reconnecting. See [Component Connection Reuse](/pages/component-connection-reuse/).
