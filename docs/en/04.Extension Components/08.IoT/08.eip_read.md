...
`x/eipRead` component: <Badge text="v0.35.0+"/> Reads controller tags in batch from Rockwell Automation Allen-Bradley ControlLogix/CompactLogix PLCs over the EtherNet/IP (CIP) protocol. Built on the [gologix](https://github.com/danomagnum/gologix) library.
...
|-----------|--------|----------------------------------------|
| name      | string | Point name                             |
| address   | string | Tag name (equals the configured/input `tag`) |
