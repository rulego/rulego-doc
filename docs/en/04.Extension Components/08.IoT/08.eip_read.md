...
`x/eipRead` component: <Badge text="v0.35.0+"/> Reads controller tags in batch from Rockwell Automation Allen-Bradley ControlLogix/CompactLogix/Micro820 PLCs over the EtherNet/IP (CIP) protocol. Built on the [gologix](https://github.com/danomagnum/gologix) library. Only CIP devices are supported; PCCC protocol devices such as PLC5, SLC, and MicroLogix are not supported.
