...
permalink: /en/pages/endpoint-modbus-server/
---

`endpoint/modbusServer`: Modbus TCP slave endpoint. Accepts master connections; writes to Coil/Holding Register trigger rule chain messages.
...
|-------|------|-------------|---------|
| listen | string | Listen URL (tcp://host:port) | `tcp://:502` |
| unitId | int | Slave unit ID (0 = accept all) | `0` |
| maxClients | uint | Max concurrent connections | `10` |
| coils | uint | Coil register count | `2000` |
| registers | uint | Register count | `2000` |
