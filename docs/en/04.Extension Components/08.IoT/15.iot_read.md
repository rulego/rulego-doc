...
`x/iotRead` component: <Badge text="v0.35.0+"/> **universal acquisition entry**. Switches the protocol backend by `driver` (modbus / s7 / eip / snmp / opcua / mc / fins / dlt645 / iec104); configuration is passed through to the corresponding protocol read node. Output is unified to `iot_points.Data`, decoupled from any specific protocol - switch backends by changing only `driver` and that protocol's connection/point config, with the rest of the chain unchanged.
