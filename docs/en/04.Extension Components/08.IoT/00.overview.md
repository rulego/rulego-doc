...
<Badge text="rulego-components-iot"/> provides industrial IoT protocol acquisition and time-series storage, covering **10 protocols + 5 TSDB backends**.
...
Use `GROUP BY name` in the aggregation SQL for per-point stats; set `inputFormat: columns` on the aggregator for cross-point calculations (wide-table pivot). See [IoT Scenario Examples](./24.scenarios.md) for a complete example.
