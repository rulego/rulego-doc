...
| [x/iotToSeries](/en/pages/x-iot-to-series/) | Acquisition output (map) → SeriesPoint transform |
...
Acquisition output is a **point array** (one name/value per point), while TSDB write needs a **map form** (measurement + fields). Use [x/iotToSeries](/en/pages/x-iot-to-series/) or a script node to pivot:
...
> Swap protocol by changing n1's `driver` (s7/opcua/snmp/fins/mc/iec104...), n2/n3 unchanged.
