...
> **提示**：本案例是一个生产级应用，涉及多智能体路由、三层记忆、心跳调度、自我进化等高级特性。建议先阅读 [架构设计](/pages/ai-agent-architecture/) 和 [开发指南](/pages/ai-agent-guide/) 了解基础概念。
...
**在线体验 & 下载**：[TPCLAW 智能助手平台](https://tpclaw.teambuf.com/)
...
如果要把智能体能力落到**审批工作流**场景，可以直接使用基于 RuleGo 构建的企业级审批工作流平台 [GFlow 审批工作流](https://gflow.rulego.cc/)：**批完，自动办**——AI 先审、人再签、签完自动执行。审批与自动化写在同一条流程 DSL 里：`aiAgent` 节点调用智能体审批，`automation` 节点调用规则链执行后续动作。
...
- [架构设计](/pages/ai-agent-architecture/) — 三层架构与核心模块
- [智能体节点](/pages/ai-agent-node/) — ReAct 节点配置详解
- [工具系统](/pages/ai-agent-tools/) — 工具类型与配置
- [切面框架](/pages/ai-agent-aspect/) — AOP 切面接口与内置切面
- [会话管理](/pages/ai-agent-session/) — 会话存储与压缩策略
- [编排案例](/pages/ai-agent-orchestration/) — 节点编排模式与示例
- [智能体组件](/pages/ai-agent/) — `ai/agent` 完整配置参考
