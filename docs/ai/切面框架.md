...
> 这些事件可通过 RuleGo-Server 的 OpenAI 兼容接口透传给外部客户端：请求头带 `X-Stream-Protocol: agui` 即在流式响应中输出工具调用过程（不带则只返回最终文本）。详见 [创建智能体 — 工具调用过程展示](/pages/rulego-server-create-agent/#工具调用过程展示ag-ui-扩展)。

## 相关文档

- [概述](/pages/ai-agent-overview/) — 框架定位与核心概念
- [架构设计](/pages/ai-agent-architecture/) — 分层架构详解
- [智能体节点](/pages/ai-agent-node/) — ReAct 节点的概念与高级特性
- [智能体组件](/pages/ai-agent/) — `ai/agent` 组件的完整配置参考
- [会话管理](/pages/ai-agent-session/) — 会话切面的底层机制
- [开发指南](/pages/ai-agent-guide/) — 自定义切面的实战应用
