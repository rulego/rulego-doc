...
流式输出中智能体需要判断模型是否发起工具调用以决定路由，而 OpenAI 兼容协议里「纯文本」和「先文本后工具调用」在发生前无法区分。默认自动模式：无工具实时流式，有工具则在首个文本后观察 500ms——窗口内出现工具调用则执行，持续纯文本则放行流式；模型前导文本超出窗口导致工具不执行时可显式设为 `drain`。相关字段：`streamToolCallCheck`，完整说明见 [智能体组件](/pages/ai-agent/)。
