...
> `ai/agent` 节点的完整字段说明，参见 [智能体组件](/pages/ai-agent/)。
>
> `ai/agent` 另有纯标准库的轻量实现（32 位平台可用），与完整版共用类型名，链 DSL 同形——详见 [智能体组件·两个同名实现](/pages/ai-agent/#两个同名实现)。
...
> 完整的 L4 智能体平台案例，参见 [应用案例](/pages/ai-agent-case-study/)。
...
- [架构设计](/pages/ai-agent-architecture/) — 分层架构、核心模块、数据流详解
...
- [编排案例](/pages/ai-agent-orchestration/) — 智能体与规则链节点组合编排的实际案例
- [应用案例](/pages/ai-agent-case-study/) — 智能助手平台完整生产案例
...
<RelatedEco :ids="['tpclaw','gflow-engine']" />
