...
::: warning 与流-流 JOIN 的 WITHIN 同名不同义
MATCH_RECOGNIZE 的 `WITHIN` 约束**模式序列的活跃期**；`JOIN ... WITHIN 30 SECONDS`（[流-流 JOIN](/pages/streamsql-stream-join/)）约束**两行时间戳的距离**。二者不能出现在同一条查询里。
:::
