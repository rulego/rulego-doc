---
title: 自定义函�?
date: 2024-01-15 10:06:00
permalink: /pages/streamsql/06/
article: false
author: 
  name: StreamSQL
...
| **数学函数** | `TypeMath` | 数值计�?| `ABS()`, `ROUND()`, `SQRT()` |
| **字符串函�?* | `TypeString` | 文本处理 | `UPPER()`, `CONCAT()`, `SUBSTRING()` |
| **转换函数** | `TypeConversion` | 类型转换 | `CAST()`, `TO_JSON()` |
| **时间函数** | `TypeDateTime` | 时间处理 | `NOW()`, `DATE_FORMAT()` |
| **通用函数** | `TypeCustom` | 通用逻辑 | 自定义业务函�?|
...
SELECT deviceId, 
       ...
       temperature
FROM stream
WHERE travel_distance > 1000  -- 距离超过1000�?
```
...
- 💡 [示例集合](../07.示例/) - 查看更多自定义函数应用案�?
- 📚 [API参考](../08.API参�?) - 完整的API文档
- 📖 [最佳实践](../09.最佳实�?) - 生产环境使用建议
- 🔙 [快速开始](../02.快速开�?) - 回顾基础用法
