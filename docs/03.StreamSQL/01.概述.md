...
- 🔗 **流-流 JOIN**：`WITHIN` 时间邻近关联两条实时流——双源信号互证、指令未回执缺席告警（LEFT 补 NULL）、事件时间对齐、3+ 流左深级联。对齐 Flink Interval Join / ksqlDB / Kafka Streams JoinWindows，状态有界适合边缘。详见[流-流 JOIN](./16.流流JOIN.md)。
...
| **复杂事件识别(CEP)** | ✅ MATCH\_RECOGNIZE | ✅ 支持         | ❌ 无          | ❌ 无        | ❌ 无          | ❌ 无          |
| **流-流 JOIN**   | ✅ WITHIN 时间邻近      | ✅ Interval Join | ❌ 无          | ✅ WITHIN    | ❌ 无          | ❌ 无          |
...
StreamSQL 为边缘而生。单实例在 **128MB 内存网关**上的吞吐（单条规则、单核，x86 实测，v1.0.3）：

| 规则                        | ns/op | allocs/op | 吞吐                |
| ------------------------- | ----- | --------- | ----------------- |
| 过滤                        | 522   | 6         | **\~192 万 msg/s** |
| 转换                        | 1359  | 12        | \~74 万 msg/s      |
| 分析 + 分区（lag PARTITION BY） | 2095  | 18        | \~48 万 msg/s      |

- **内存不是瓶颈**：128MB 可承载 **10 万+ 设备**的分区状态（实测 5 万分区仅占 4.5MB 堆）。
- **CPU 吞吐才是**：一条规则吃满一个核是最优用法；多核靠并行多个独立实例扩展（调高 `GOGC` 可近线性）。
- **稳定有界**：无 goroutine 泄漏，堆不随负载/分区数线性增长。
- **典型容量**（A53 低端 128MB 网关）：每设备 1 条/秒约带 25–32 万设备；每设备 100 条/秒约带 2500–3200 设备（过滤规则）。

> ARM 网关数字为 x86 折算估算（A53 低端过滤 \~24–32 万 msg/s），**上线前需在目标 SoC 实测**。基准代码：`test/e2e/stress_test.go` 的 `BenchmarkGateway_*`。
...
上面三个场景都是「时间段统计」，用窗口聚合即可。但有两类需求窗口聚合做不了，是 StreamSQL 区别于其他轻量引擎的独门绝技：

**① 连续越限防抖告警（CEP）**　单次越限可能是传感器抖动或瞬时尖峰，连续 N 次才是真异常：

```sql
-- 温度连续 3 次越过 80℃ 才告警（防抖），10 分钟内必须凑齐
SELECT * FROM stream
MATCH_RECOGNIZE (
...
**② 设备开停机工作流识别（CEP）**　识别"开机→运行→停机"的有序序列，关心事件**顺序**而非统计：

```sql
-- 识别完整工作流，输出每次运行的起止时间与峰值温度
SELECT * FROM stream
MATCH_RECOGNIZE (
...
**③ 状态变化检测（分析函数 / CDC）**　只在指标真正变化时输出，并带上一次的值，滤掉冗余重复：
...
**何时用什么**：相邻事件比较（变没变）→ 分析函数；事件顺序/序列模式（A 之后接 B、连续 N 次）→ CEP；时间段统计（每分钟均值）→ 窗口聚合 + HAVING。详见[模式识别](./15.模式识别.md)与[分析函数](./13.分析函数.md)。
...
- 🧩 [模式识别](/pages/streamsql-cep/) - MATCH\_RECOGNIZE 事件序列模式识别（CEP）
- 🔗 [流-流 JOIN](/pages/streamsql-stream-join/) - 双流时间邻近关联与缺席告警
