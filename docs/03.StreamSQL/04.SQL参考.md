...
**作用**：为 `CountingWindow` 设置分组状态的存活时间（TTL），定期清理长期不活跃的"死 key"缓冲，防止高基数 `GROUP BY` 场景下内存无限增长。
...
**重要说明**：
- 仅作用于 `CountingWindow`（按 count 触发，本身不基于时间，状态不会被自然清理）
...
| 每个 key 的数据稀疏（达不到 count 阈值就不再来数据） | **推荐配置** |
| 时间窗口（Tumbling/Sliding/Session） | 不适用（仅 CountingWindow） |
...
| `STATETTL` | string | 计数窗口状态 TTL（仅 CountingWindow） | `'0s'`（禁用） | 否 |
