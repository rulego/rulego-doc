...
| LIMIT | 否 | 限制结果数量 |
| WITH | 否 | 配置选项 |
| JOIN | 否 | 流表 JOIN（元数据富化）或流-流 JOIN（`WITHIN`，时间邻近关联）——见 [JOIN 子句](#join-子句) |
...
JOIN 支持两种形态，**用是否带 `WITHIN` 区分**：

**① 流表 JOIN（元数据富化，无 WITHIN）**——流 × 注册表（`RegisterTable`），按当前表快照即时挂载：
...
**② 流-流 JOIN（时间邻近关联，WITHIN 必填，v1.3.0+）**——两条实时流按键关联且 `|L.ts − R.ts| ≤ WITHIN`，对齐 Flink Interval Join / ksqlDB / Kafka Streams JoinWindows：
...
- `WITHIN 30 SECONDS` / `WITHIN '30s'` / `WITHIN 100 MS` 等价；位置在 JOIN 表名之后或 ON 之后（二选一）；
- JOIN 类型：INNER / LEFT（LEFT 无匹配在窗口关闭时补 NULL 行）；RIGHT/FULL/CROSS 报错；
- 事件时间：`WITH (TIMESTAMP = 'ts')`；不写按到达时间；
- ON 仅等值条件，复合键 AND 拼接；3+ 流按左深级联，每级独立 WITHIN；
- 不支持：WITHIN JOIN 与 GROUP BY / 窗口 / HAVING / DISTINCT / LIMIT / 分析函数 / 流表 JOIN / unnest 组合（编译期报错）。

完整语义、资源边界与更多示例见[流-流 JOIN](./16.流流JOIN.md)。
...
- `存储过程` （PROCEDURE）
- `触发器` （TRIGGER）
- `RIGHT/FULL/CROSS JOIN`（JOIN 支持[流表 JOIN](#join-子句)与 `WITHIN` 流-流 JOIN 两种形态）

### 限制说明

1. **数据源**：单流查询单一数据源；多数据源仅限 JOIN 形态（流-流 JOIN 按流名喂入）
2. **无持久化**：不支持数据持久化存储
3. **无事务**：不支持事务操作
4. **内存限制**：受限于单机内存容量（流-流 JOIN 状态由 WITHIN 保留期 + LRU/行数闸限定）
