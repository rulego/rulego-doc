...
- **分析函数**（`lag` / `had_changed` / `changed_col` / `changed_cols` / `latest` / `acc_*`）属于**非聚合**查询——逐事件维护跨事件状态、每条数据到达即求值，可用在 `SELECT` 和 `WHERE`，典型用途是变化检测（CDC）。详见 [分析函数](../13.分析函数.md)。
- 分析函数**可用于窗口查询**（对窗口产出行求值，参数可为聚合函数），但**不能进 `HAVING`**；时间段/持续检测用窗口聚合 + `HAVING`（见[滑动窗口与持续检测](./31.案例集锦/04.滑动窗口与持续检测.md)）。
