...
- **处理时间窗口对齐到 epoch 边界**（与事件时间一致）：`TumblingWindow('1m')` 的窗口结束在整分钟（如 10:01:00、10:02:00），**与首条数据到达时刻无关**。这与 Flink `TumblingProcessingTimeWindows` 和 eKuiper（"align to the nature time ... regardless of the rule start time"）一致
...
- 基于数据间隔判断会话
- 适合用户行为分析
- 当前 session 窗口**仅做尾部扩展**（gap 内新事件延长会话 End），**不做跨 session 合并**（Flink 的 merging）。这与 eKuiper 的 session 实现一致；仅事件时间 + 严重乱序数据下与 Flink 行为不同
