...
- 非聚合（过滤 / 转换 / 分析函数）→ `EmitSync`；聚合 / 窗口 → `Emit` + `AddSink`。两套 API 详见[核心概念 · 两种执行路径](./03.核心概念.md#两种执行路径与-api)。
- 变化检测（CDC）、`lag`、累积统计等见[分析函数](./13.分析函数.md)。
:::
...
::: tip 进阶用法
自定义函数（UDF）、性能模式（高吞吐 / 低延迟）等进阶用法见 [进阶示例](./14.进阶示例.md)。
:::
