...
::: warning 限制
当前版本：流表 JOIN 仅用于流数据富化（enrichment），运行在非窗口路径上。**流表 JOIN 与聚合/窗口组合会被拒绝**（`Execute` 返回 `"JOIN with aggregation/window is not supported"`）。如需窗口聚合，请去掉 JOIN。（v1.3.0 起另有带 `WITHIN` 的流-流 JOIN，见下一节，不受此限。）
:::
...
流-流 JOIN（v1.3.0+）在 SQL 中用 `WITHIN` 声明（语法与语义见[流-流 JOIN](./16.流流JOIN.md)），Go 侧通过以下 API 按流名喂入数据。全部为新增 API，既有红线 API 零变化。
...
- `stream` — 流名，即 SQL 中 `FROM` / `JOIN` 后的名字，**大小写敏感**；
- 未知名返回 error（错误信息含已知流名列表）；
- `Emit(row)` 等价于 `EmitTo(FROM 流名, row)`，单流习惯代码无需修改；
- 仅对流-流 JOIN（WITHIN）查询可用；其他查询调用返回 error（提示改用 `Emit`）。
...
判定当前查询是否为流-流 JOIN 查询（`Execute` 之后调用）。供 RuleGo 组件等多源接入层路由：为真时按元数据中的流名调 `EmitTo`。

### WithJoinMaxKeys / WithJoinMaxRows

```go
func WithJoinMaxKeys(n int) Option  // 每侧每级缓冲 key 上限（LRU，默认 10000）
func WithJoinMaxRows(n int) Option  // 每侧每级缓冲行上限（默认 0=无界）
```
...
ssql.AddSink(func(rows []map[string]interface{}) { /* 告警行 */ })
...
ssql.Stop() // 未匹配的 LEFT pending 行补发 NULL 后退出
```

::: warning
`EmitSync` 对流-流 JOIN 查询返回 error（同 CEP 查询）；结果只能经 `AddSink` / `ToChannel` 异步获取。
:::
