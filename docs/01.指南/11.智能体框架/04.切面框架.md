...
> 这些事件可通过 RuleGo-Server 的 OpenAI 兼容接口透传给外部客户端：请求头带 `X-Stream-Protocol: agui` 即在流式响应中输出工具调用过程（不带则只返回最终文本）。详见 [创建智能体 — 工具调用过程展示](../12.RuleGo-Server/20.可视化编辑器/04.创建智能体.md#工具调用过程展示ag-ui-扩展)。
