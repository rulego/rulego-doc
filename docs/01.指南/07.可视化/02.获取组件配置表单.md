...
该API会扫描所有注册到注册器的组件，根据[约定方式](/pages/component-form-conventions/) 得到组件定义及其表单配置信息。用于规则链可视化配置，组件物料加载。
...
示例返回结果：[doc/components.json](https://github.com/rulego/rulego/tree/main/doc/components.json)
...
| version       | string               | 组件版本                       | 空                                                                         |
| componentKind | string               | 组件类别：`nc`(内置)、`dc`(动态)、`ec`(endpoint) | 自动检测                                                                      |
| router        | RouterForm           | 路由器表单配置，仅 endpoint 组件使用     | 空                                                                         |

### types.RouterForm

仅 Endpoint 组件使用，定义路由器的可视化表单配置。

| 字段 | 类型 | 说明 |
|------|------|------|
| hide | bool | 是否隐藏路由器配置。`true` 时自动生成默认路由（`from.path="*"`），如定时调度组件 |
| from | RouterFormField | 路由源配置（如 HTTP 路径、MQTT Topic） |
| to | RouterFormField | 路由目标配置 |
| defaultValue | []map | 默认路由配置 |
...
| required     | bool                 | 是否必填                                            | false，可以通过tag:required指定                   |
...
| ref          | string               | 共享节点池关联：`primary`=引用字段，`shared`=共享节点提供，空=组件自身字段 | 空                                           |
...
自定义组件也可以通过实现以下可选接口，覆盖[组件配置表单约定](/pages/component-form-conventions/) 的定义信息:
