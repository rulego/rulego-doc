...
//ReloadSelf 的入参是完整的规则链DSL（不是增量片段），未包含的节点和连接会被删除
updateRuleChainFile := `{
  "ruleChain": {
    "id":"chain_call_rest_api",
    "name": "测试规则链",
...
`
//热更新规则引擎实例：变更节点s2，把温度加5，消息处理立即生效，无需重启应用
_ = ruleEngine.ReloadSelf([]byte(updateRuleChainFile))
```

## 其他

1. 我们内置了大量的[标准组件](/pages/standard-components/) 和 [扩展组件](/pages/extension-overview/) ，你只需要通过配置的方式组合这些组件，实现你的功能需求。
2. 你也可以很容易地开发自己的组件，参考 [自定义组件](/pages/custom-components-overview/) 。
