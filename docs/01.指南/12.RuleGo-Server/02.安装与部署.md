...
> **`use_fasthttp` 选型建议**：fasthttp 在 **4 核及以上**机器上吞吐提升显著（实测 +47~82%），但在 **2 核机器上反而慢约 30%**（fasthttp 端点框架的连接管理机制在少核下成为负担）。2 核及以下机器建议使用默认的 net/http 实现。用法示例：
>
> ```bash
> # 4 核以上机器：启用 fasthttp 获得更高吞吐
> go build -tags "with_all,use_fasthttp" -o server ./cmd/server/
>
> # 2 核机器：使用默认 net/http
> go build -tags "with_all" -o server ./cmd/server/
> ```
