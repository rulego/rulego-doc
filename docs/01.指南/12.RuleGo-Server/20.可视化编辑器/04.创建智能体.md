...
智能体在服务端自主执行工具。默认情况下接口只返回最终文本（`finish_reason: "stop"`），**不下发 `tool_calls`**——因为 `tool_calls` 在 OpenAI 协议里表示"由客户端执行"，下发会让 Claude Code、Cursor、Cherry Studio 等标准客户端去执行服务端已经执行过的工具，造成重复执行或报错。

**何时启用**：客户端需要流式展示完整的工具调用过程（调用、参数、结果）时，在请求头带 `X-Stream-Protocol: agui` 切换到 AG-UI 扩展模式。不带该头则保持标准 OpenAI 响应，标准客户端无需任何改动。
...
扩展模式下，工具调用事件放在流式响应的 `choices[0].delta.tool_calls[0]` 中：

| 字段 | 说明 |
|------|------|
| `type` | 事件类型：`TOOL_CALL_START`（开始）/ `TOOL_CALL_RESULT`（结果） |
| `toolCallId` | 工具调用唯一 ID |
| `toolCallName` | 工具名称 |
| `arguments` | 工具参数（JSON 字符串，`TOOL_CALL_START` 时携带） |
| `content` | 工具执行结果（`TOOL_CALL_RESULT` 时携带） |
| `toolType` | 工具类型：`builtin` / `rulechain` / `subagent` / `mcp` |

> RuleGo-Editor 内置聊天面板默认带该头，可实时展示工具调用卡片。第三方 OpenAI 客户端不带该头，接口返回标准 Chat Completions 响应。
