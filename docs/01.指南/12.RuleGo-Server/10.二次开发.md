...
| `KeyUserAdmin` | `module.user.admin` | `UserAdmin` | 用户管理（增删改查） |
...
| `KeyDebugService` | `module.debug.service` | `DebugService` | 调试服务（由 runlog 模块注册） |
| `KeyIoTPointService` | `module.iotpoint.service` | `PointTemplateService` | IoT 点位模板 |
...
type Authenticator interface {
    // 解析 authorization 头（"Bearer xxx" 原始值），返回 UserContext，失败返回 error
...
type Authorizer interface {
    // 检查 user 对 resource 能否做 action，有权限返回 nil，否则返回 error
...
| `use_fasthttp` | 用 fasthttp 替换 HTTP 端点和 restApiCall 组件。短连接/高并发场景吞吐提升约 20%~30%、p99 降低约 40%，常规 keep-alive API 提升约 7%~10%，按负载特征选型 |

```bash
# 启用 fasthttp 端点（需包含修复版 rulego-components；建议 CGO_ENABLED=0 静态编译）
