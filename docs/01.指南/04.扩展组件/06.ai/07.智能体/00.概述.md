...
将上述 JSON 与智能体开发概念一一对应：

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  ruleChain（规则链 = 智能体容器）                                              │
│  ├── id: "weather-agent"          → 智能体唯一标识，用于 API 调用              │
│  ├── name: "天气助手"              → 智能体显示名称                            │
│  │                                                                            │
│  └── metadata                                                                                 │
│      ├── firstNodeIndex: 0        → 入口节点索引（从哪个节点开始执行）           │
│      │                                                                            │
│      ├── nodes[]（节点数组 = 智能体的能力单元）                                  │
│      │   ├── [0] ai/agent         → AI 智能体节点（核心：ReAct 推理循环）       │
│      │   └── [1] end              → 结束节点（必须有）                          │
│      │                                                                            │
│      └── connections[]（连接 = 节点间的流转关系）                                │
│          ├── agent → end (Success) → 同步成功时流转到结束                       │
│          └── agent → end (Stream)  → 流式输出时流转到结束                       │
└─────────────────────────────────────────────────────────────────────────────┘
```

**ai/agent 节点的 configuration 字段详解：**

| 字段 | 含义 | 对应智能体概念 |
|------|------|---------------|
| `url` | LLM API 地址 | 模型服务端点 |
| `key` | API 密钥 | 认证凭证 |
| `model` | 模型名称 | 使用哪个 LLM（如 gpt-4o、GLM-5） |
| `systemPrompt` | 系统提示词 | 智能体的"人设"和行为规则，支持 `${include()}` 从文件加载 |
| `maxStep` | 最大推理步数 | 防止无限循环的安全限制 |
| `tools[]` | 工具数组 | 智能体可调用的能力 |
| `params` | 模型参数 | temperature、topP 等调优参数 |

**tools 数组中每个工具的结构：**

| 字段 | 含义 | 示例 |
|------|------|------|
| `type` | 工具类型 | `builtin`（内置）、`mcp`、`rulechain`、`agent` |
| `name` | 工具名称 | `bash`、`read`、`write`、`edit`、`skill` |
| `config` | 工具配置 | 每个工具特有的参数（如 `workDir`） |

**connections 数组中每个连接的结构：**

| 字段 | 含义 | 可选值 |
|------|------|--------|
| `fromId` | 源节点 ID | 节点的 `id` |
| `toId` | 目标节点 ID | 节点的 `id` |
| `type` | 连接类型 | `Success`（成功）、`Stream`（流式）、`Failure`（失败） |
...
| 系统提示词 | `nodes[0].configuration.systemPrompt` | 定义智能体行为 |
| 工具集 | `nodes[0].configuration.tools[]` | 智能体可调用的能力 |
| 模型配置 | `nodes[0].configuration.url/key/model` | 使用哪个 LLM |
| 执行结果 | `connections[type=Success/Stream]` | 成功/流式输出流转到哪里 |
| 错误处理 | `connections[type=Failure]` | 失败时流转到哪里 |
| 多智能体编排 | 多个 `ai/agent` 节点 + `connections` | 通过连接实现智能体间协作 |
| 业务逻辑集成 | 其他规则链节点（jsFilter、restApiCall 等） | 智能体与业务系统联动 |

### 两种使用模式

**基础模式（推荐）**：只配置 `ai/agent` 节点 + 工具/skill，让智能体自主编排完成任务。适用于大多数场景，如编程助手、问答系统、内容生成等。

```json
{
  "nodes": [
    { "id": "node_agent", "type": "ai/agent", "configuration": { "tools": [...] } },
...
**高级模式**：智能体节点后接 RuleGo 生态的其他节点（jsFilter、restApiCall、jsTransform 等），实现可编排的工作流。适用于：

- **小模型意图编排**：用小模型（如 GLM-4-Flash）做意图分类，`maxStep=1` 不调用工具，输出结构化 JSON，后接路由节点分发到不同处理链
- **多步处理管道**：LLM 生成 → 数据清洗 → 格式转换 → API 调用
- **条件路由**：根据 LLM 输出的字段值，路由到不同的下游节点

```json
{
  "nodes": [
    { "id": "node_agent", "type": "ai/agent", "configuration": { "maxStep": 1, "tools": [] } },
    { "id": "node_route", "type": "jsFilter", "configuration": { "jsScript": "..." } },
    { "id": "node_api", "type": "restApiCall", "configuration": { "url": "..." } },
    { "id": "node_end", "type": "end" }
  ],
  "connections": [
    { "fromId": "node_agent", "toId": "node_route", "type": "Success" },
    { "fromId": "node_route", "toId": "node_api", "type": "True" },
    { "fromId": "node_route", "toId": "node_end", "type": "False" }
  ]
}
```

> 更多高级编排案例，参见 [智能体编排案例](./07.编排案例.md)。
