...
[RuleGo AI](https://github.com/rulego/rulego-components-ai) 智能体开发框架是一套基于 [RuleGo](https://github.com/rulego/rulego) 构建的声明式智能体开发框架。它将 AI 智能体定义为可编排的规则链，融合了 LLM 推理能力与规则引擎的确定性编排能力，提供了工具系统、AOP 切面、会话管理和 MCP 集成等企业级特性。
...
使用 `ai/llm` 组件，无工具调用，适合意图分类、内容生成等场景。

### L2 — 工具增强对话

使用 `ai/agent` + 内置工具（bash/read/write/edit），智能体可以读写文件、执行命令、自主完成任务。这是最常见的智能体形态。

### L3 — 多智能体协作

通过 `agent` 类型工具将多个智能体组合，主智能体可以委派任务给专业子智能体（代码审查、测试生成、文档编写等），实现分工协作。

### L4 — 自主编排智能体

类似 Claude Code、OpenClaw、Hermes 等高级智能体的形态。框架提供的完整基础设施支撑这类智能体的开发：

| 能力 | 框架支撑 | 说明 |
|------|----------|------|
| **自主规划与执行** | ReAct 循环 + maxStep 控制 | 智能体自主决定调用哪些工具、以什么顺序执行 |
| **文件读写与编辑** | read/write/edit 内置工具 | 支持代码生成、文件修改、项目管理 |
| **Shell 命令执行** | bash 工具 + 安全策略 | 执行构建、测试、部署等命令，支持白/黑名单 |
| **技能扩展** | skill 技能系统 | 通过 SKILL.md 文件定义可复用的专业能力 |
| **工作区隔离** | workDir 配置 | 每个智能体独立的工作目录，文件系统级隔离 |
| **会话记忆** | Session 模块 + SessionAspect | 多轮对话历史管理、自动压缩、长期记忆 |
| **自我进化** | 工作区文件 + write/edit 工具 | 智能体可以修改自身的行为规则、记忆和技能 |
...
| **心跳调度** | 外部 HeartbeatService | 定时触发智能体主动执行任务（如检查待办、发送通知） |

以 Claude Code 类智能体为例，基于本框架的实现方式：

```
workspace/
  IDENTITY.md      → 智能体身份定义（"你是一个编程助手"）
  AGENTS.md        → 行为规则（代码风格、提交规范）
  SOUL.md          → 核心原则（简洁、安全、可测试）
  TOOLS.md         → 工具使用指南
  MEMORY.md        → 长期记忆
  HEARTBEAT.md     → 待办任务队列
  skills/          → 专业技能（代码审查、测试生成）
```

系统提示词通过 `${include()}` 动态加载这些文件，智能体通过 `write`/`edit` 工具可以修改它们，实现自我进化。
