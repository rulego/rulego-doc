...
平台支持一种特殊的智能体模式——**意图路由智能体**，使用小模型做文本归一化，根据输出分支处理：

```
用户输入
    ↓
[ai/agent] 小模型文本归一化（maxStep=1，无工具）
    ↓ 输出: "CHAT: 你好" 或 "开灯"
[jsFilter] 路由判断（是否以 "CHAT:" 开头）
    ├── True（命令）→ [jsTransform] 构造请求体 → [restApiCall] 执行指令
    └── False（闲聊）→ [jsTransform] 清理聊天前缀 → 返回
    ↓
结束
```
...
```json
{
  ...
  "metadata": {
    ...
    "nodes": [
      {
        ...
        "configuration": {
          ...
          "systemPrompt": "${include(global.root_dir+'/workspace-agent-device/AGENTS.md')}",
          "tools": []
        }
      },
      {
        ...
        "name": "路由判断",
        "configuration": {
          "jsScript": "if(typeof msg==='string'){return !msg.startsWith('CHAT:');} return true;"
        }
      },
      {
        "id": "node_wrap",
        ...
        "name": "构造请求体",
        "configuration": {
          "jsScript": "var text=typeof msg==='string'?msg:String(msg); return{msg:JSON.stringify({userContent:text}),metadata:metadata,msgType:msgType,dataType:'JSON'};"
        }
      },
      {
        "id": "node_execute",
        ...
        "configuration": {
          "requestMethod": "POST",
          "restEndpointUrlPattern": "http://your-device-server/api/v1/command",
      ...
      },
      {
        ...
        "name": "清理聊天输出",
        "configuration": {
          "jsScript": "var text=typeof msg==='string'?msg:''; if(text.startsWith('CHAT:')){text=text.substring(6);} return{msg:text,metadata:metadata,msgType:msgType,dataType:'TEXT'};"
        }
      },
      {
        "id": "node_end",
    ...
    "connections": [
      { "fromId": "node_normalize", "toId": "node_route", "type": "Success" },
      { "fromId": "node_normalize", "toId": "node_end", "type": "Stream" },
      { "fromId": "node_normalize", "toId": "node_end", "type": "Failure" },
      { "fromId": "node_route", "toId": "node_wrap", "type": "True" },
      { "fromId": "node_wrap", "toId": "node_execute", "type": "Success" },
      { "fromId": "node_route", "toId": "node_chat_clean", "type": "False" },
      { "fromId": "node_execute", "toId": "node_end", "type": "Success" },
      { "fromId": "node_execute", "toId": "node_end", "type": "Failure" },
      { "fromId": "node_chat_clean", "toId": "node_end", "type": "Success" }
    ]
  }
}
```
...
| `tools` | 5 个工具 | 空 |
| `temperature` | 0.7 | 0.3 |
| 后续节点 | 直接结束 | jsFilter 路由 → restApiCall |
| 适用场景 | 通用对话+工具使用 | 小模型分类+指令执行 |
...
本文档描述的智能助手平台已在生产环境完整落地，是一个开箱即用的商业产品。你不需要从零搭建——购买后即可获得：

- **完整源码**：Go 后端 + Vue 前端，可私有化部署
- **开箱即用的智能体**：主智能体、意图路由智能体等多种模板
- **IM 全通道接入**：飞书、企业微信已深度集成，扫码即用
