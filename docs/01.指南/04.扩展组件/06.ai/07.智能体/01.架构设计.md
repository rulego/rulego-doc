...
| 层级 | 职责 | 代表组件 |
|------|------|----------|
| **应用层** | 业务逻辑、渠道接入、用户管理 | tpclaw、自定义切面、API 路由 |
...
```
BaseModel → RetryWrapper → DynamicWrapper → AgentAwareWrapper
```

工具对象通过 `VisualToolWrapper` 增强横切能力。装饰器模式确保核心逻辑与增强功能解耦。

### 责任链模式（Chain of Responsibility）

Around 切面按逆序构建责任链，每个切面可以决定是否调用 `next` 继续执行，实现超时控制、重试、缓存等功能。
...
| 注册表 | 管理对象 | 用途 |
|--------|----------|------|
| `ToolRegistry` | 内置工具 | 工具发现和实例化 |
| `AspectRegistry` | 切面实例 | 全局切面注册 |
| `EmitterRegistry` | 事件发射器 | 规则链级别的事件推送 |

### 策略模式（Strategy）

工具创建根据 `type` 字段选择不同的创建策略（builtin/rulechain/agent/mcp），MCP 工具内部进一步区分 self 和远程策略。
...
sequenceDiagram
    ...
    Aspect->>LLM: Generate/Stream<br/>(ReAct 推理)
