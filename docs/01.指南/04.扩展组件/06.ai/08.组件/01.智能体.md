...
| streamRetryMode | string | 流式 mid-stream 重试模式：`off`（默认，仅探测窗口内重试，保留实时）/ `full`（完整缓冲重放，牺牲实时换中途断流可重试） | off |
| streamToolCallCheck | string | 流式工具调用判定模式：空=自动（默认）；`firstContent`=首个文本即判定；`drain`=读完流再判定。详见 [流式工具调用判定](#流式工具调用判定streamtoolcallcheck) | |
