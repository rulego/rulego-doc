...
| [x/iotToSeries](/pages/x-iot-to-series/) | 采集输出(map) → SeriesPoint 转换 |

### 协议读写节点

| 协议 | 读节点 | 写节点 | 地址格式 |
|------|--------|--------|----------|
| Modbus | `x/modbusRead` | `x/modbusWrite` | Modicon（1-based）：`40001`=保持寄存器、`30001`=输入寄存器、`00001`=Coil、`10001`=离散输入 |
...
采集输出是**点数组**（每点一个 name/value），时序写入需要 **map 形式**（measurement + fields）。使用 [x/iotToSeries](/pages/x-iot-to-series/) 或脚本节点做透视转换：
