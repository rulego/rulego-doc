...
| parity             | string | 校验位（仅RTU模式），可选值：<br>`N` - 无校验<br>`E` - 偶校验<br>`O` - 奇校验<br><br>全称 `none`/`even`/`odd` 亦可，大小写不敏感。<br>为兼容早期配置，数值 `0`/`1`/`2` 仍可读入（依次对应 N/E/O），但导出与文档统一用字母。<br><br>注：字母形式对齐现场惯用写法（如 `8E1`）。RTU 驱动不支持 Mark/Space 校验，填 `M`/`S` 会明确报错而非静默回落——需要这两种请用 [x/serial](./04.serial.md) 组件。 | `N`                   |
