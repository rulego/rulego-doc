...
`x/modbusReadPoints`组件：<Badge text="v0.35.0+"/> 通过 Modbus 协议（TCP/RTU）批量读取点位，结果以**统一契约** `iot_points.Data` 列表写回 `msg.Data`，成功走 `Success` 链、失败走 `Failure` 链。
...
- 写入：[x/modbusWritePoints](/pages/x-modbus-write-points/)
