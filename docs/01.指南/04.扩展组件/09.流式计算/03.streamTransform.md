...
### 单条数据输入
直接处理单个JSON对象：转换成功走 Success，不符合 WHERE 条件走 Filtered，处理出错走 Failure：
```json
...
- 全部元素被 WHERE 过滤（无出错）走 Filtered；存在出错元素走 Failure
...
- **Filtered：** 数据被过滤（WHERE 不满足，或 `changed_cols`/`changed_col` 无变化），**非错误**；metadata 设 `match=false`
- **Failure：** 处理出错（非 JSON 数据、SQL 求值错误等），通过此关系链传递错误信息

::: tip 区分"过滤"和"错误"
被 WHERE 过滤或 `changed_cols` 无变化是**预期的不输出**，走 **Filtered**；只有真正的处理错误才走 **Failure**。所以 Failure 链可以放心接告警/错误处理，不会混入正常的事件压缩。
:::
...
::: tip "无变化"走 Filtered 链
`changed_cols`/`changed_col` 作为唯一输出且本次无变化时，节点返回 nil → 走 **Filtered** 链（`match=false`）。这是预期的事件压缩——接 Filtered 链丢弃/忽略即可，不会再混进 Failure。
:::
...
{temperature:23, humidity:55} → 走 Filtered 链（都未变，整行抑制）
```

::: tip 无变化走 Filtered 链
`changed_cols` 作为唯一输出且本次无变化时返回 nil → 走 **Filtered** 链（`match=false`）。这是预期的事件压缩，接 Filtered 链丢弃/忽略即可。
:::
...
5. **WHERE条件：** 不符合WHERE条件的数据会被过滤掉（走 Filtered 链），不包含在输出结果中
