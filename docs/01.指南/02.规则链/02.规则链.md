...
用于配置规则链的自动触发器，例如可以配置：mqtt、http、ws、schedule等接入端点触发器，详细参考：[Endpoint DSL](/pages/endpoint-dsl/) 


### 示例

#### 示例1：无 Endpoint 的规则链（被动调用）

这种模式下，规则链通过 API 调用或其他规则链的 flow 组件触发执行：
...
#### 示例2：有 Endpoint 的规则链（主动触发）

这种模式下，Endpoint 自动接收外部数据并触发规则链执行：
    ...
    "nodes": [
      {
        ...
        "name": "数据转换",
        "configuration": {
          "jsScript": "var data = JSON.parse(msg); data.processed = true; return {'msg':data,'metadata':metadata,'msgType':msgType};"
        },
        "additionalInfo": {
          "layoutX": 400,
          "layoutY": 300
      ...
      },
      {
        ...
        "name": "推送结果",
        "configuration": {
          "restEndpointUrlPattern": "http://backend:9099/api/result",
          "requestMethod": "POST"
        },
        "additionalInfo": {
          "layoutX": 600,
...
访问 `POST http://localhost:8080/api/data` 即可触发此规则链执行。
