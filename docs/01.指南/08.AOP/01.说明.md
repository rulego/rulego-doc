...
- **切面（Aspect）：** 切入点和增强点的集合。例如：[Debug Aspect](/pages/debug-aspect/) 它实现了Before Advice和After Advice。
    - 框架内置部分切面，例如：[Debug Aspect](/pages/debug-aspect/)、[Fallback Aspect](/pages/fallback-aspect/)。
    - 你可以根据需要实现自定义切面。
